...
var PayIslandCheckout = (() => {
  ...
  function extractChannels(payload) {
    ...
    if (Array.isArray(candidates) && candidates.length > 0) {
      const channels2 = candidates.filter(
        (channel) => typeof channel === "string"
      ).map(normalizePaymentChannel);
      return [...new Set(channels2)];
    }
    const channels = [];
...
`;
  ...
  var CheckoutModal = class {
    ...
    filterChannels(channels, allowedChannels, payload) {
      ...
      if (allowedChannels?.length) {
        ...
        return unique.filter(
          (channel) => allowed.includes(channel) && (payload ? this.isSupported(channel, payload) : true)
        );
      }
    ...
    handleClick(event) {
      ...
      const initialize = target.closest(
        '[data-action="initialize-channel"]'
  ...
  function buildCardFrameUrl(payload) {
    const transaction = extractTransaction(payload);
    const reference = typeof transaction.reference === "string" && transaction.reference.trim() ? transaction.reference.trim() : typeof payload.reference === "string" && payload.reference.trim() ? payload.reference.trim() : void 0;
    if (!reference) return void 0;
    const rawAuthorizationUrl = extractAuthorizationUrl(payload);
    if (!rawAuthorizationUrl) return void 0;
  ...
  function isTrustedCardFrameOrigin(origin) {
    try {
      ...
      return url.protocol === "https:" && (url.hostname === "checkout.payislands.com" || url.hostname === "app-demo.payislands.com" || url.hostname === "checkout-demo.payislands.com") || url.hostname === "localhost" || url.hostname === "127.0.0.1";
    } catch {
      return false;
    }
  }
  ...
  function navigateToReceiptPage(checkout, transaction) {
    if (typeof window === "undefined") return;
    const reference = typeof transaction.reference === "string" && transaction.reference.trim() ? transaction.reference : checkout.reference;
    const url = new URL(
      "/payment-status",
    ...
    const navigate = checkout.options.__navigate ?? ((target) => window.location.assign(target));
...
//# sourceMappingURL=payisland-checkout.js.map
