"use strict";var PayIslandCheckout=(()=>{var R=Object.defineProperty;var de=Object.getOwnPropertyDescriptor;var pe=Object.getOwnPropertyNames;var le=Object.prototype.hasOwnProperty;var ue=(t,e)=>{for(var n in e)R(t,n,{get:e[n],enumerable:!0})},fe=(t,e,n,r)=>{if(e&&typeof e=="object"||typeof e=="function")for(let o of pe(e))!le.call(t,o)&&o!==n&&R(t,o,{get:()=>e[o],enumerable:!(r=de(e,o))||r.enumerable});return t};var he=t=>fe(R({},"__esModule",{value:!0}),t);var Ie={};ue(Ie,{close:()=>k,default:()=>Le,open:()=>oe});var me="https://ags.payislands.com",ge=new Set(["paid","successful","success"]),ye=new Set(["failed","canceled","cancelled","reversed"]),be=new Set(["expired"]),ve=new Set(["pending","unpaid","processing","initiated"]),p=class extends Error{constructor(e){super(e.message),this.name="CheckoutError",this.code=e.code,this.status=e.status,this.requestId=e.requestId}toPayload(){return{code:this.code,message:this.message,status:this.status,requestId:this.requestId}}};function j(t){if(typeof t!="string"||t.trim().length<6)throw new p({code:"invalid_reference",message:"Enter a valid PayIsland transaction reference."});let e=t.trim();if(!/^[a-zA-Z0-9_-]+$/.test(e))throw new p({code:"invalid_reference",message:"The PayIsland transaction reference contains unsupported characters."});return e}function A(t){if(!t)return me;let e=window.location.hostname;if(!["localhost","127.0.0.1","::1"].includes(e))throw new p({code:"api_base_not_allowed",message:"API base overrides are only available for local development and testing."});return t.replace(/\/+$/,"")}function K(t){return t.get("x-request-id")??t.get("x-correlation-id")??t.get("x-trace-id")??void 0}function U(t){let e=String(t??"").trim().toLowerCase();return ge.has(e)?"success":be.has(e)?"expired":ye.has(e)?"failed":ve.has(e)?"pending":"unknown"}function l(t){if(!t)return{};let e="data"in t?t.data:void 0;return("transaction"in t?t.transaction:void 0)??e??t}function E(t){let e=l(t),n=typeof t?.status=="string"?t.status:void 0;return e.status??e.payment_status??n}function F(t){let e=l(t),n=t?.channels??t?.payment_channels??t?.available_channels??e.channels;if(Array.isArray(n)&&n.length>0)return n.filter(o=>typeof o=="string");let r=[];return h(t)&&r.push("bank-transfer"),_(t)&&r.push("redirect"),r.length>0?r:["bank-transfer","redirect","card"]}function J(t){let e=l(t),n=t?.business??e.business??{};return t?.merchant??e.merchant??{business_name:t?.business_name??e.business_name??t?.merchant_name??e.merchant_name??n.business_name??n.merchant_name??n.name,name:t?.merchant_name??e.merchant_name??n.name??n.business_name,logo:t?.business_logo??t?.merchant_logo??e.business_logo??e.merchant_logo??n.business_logo??n.merchant_logo??n.logo_url??n.logo}}function G(t){let e=l(t);return t?.customer??e.customer??{}}function h(t){if(!t)return;let e=M(t);if(e)return e;let n=l(t);if(n!==t)return M(n)}function _(t){if(!t)return;let e=t.authorization_url??t.authorizationUrl;if(e)return e;let n=l(t);if(n!==t)return n.authorization_url??n.authorizationUrl}function g(t,e=5e3){let n=t?.retry_after_ms,r=t?.poll_interval_ms,o=typeof n=="number"?n:r;return!o||o<1e3?e:Math.min(o,6e4)}function y(t,e="NGN"){if(t==null||t==="")return"--";let n=Number(t);return Number.isFinite(n)?new Intl.NumberFormat("en-NG",{style:"currency",currency:e,minimumFractionDigits:2}).format(n):String(t)}function W(t){if(!t)return"";let[e,n]=t.split("@");return!e||!n?t:`${e.slice(0,2)}${"*".repeat(Math.max(e.length-2,2))}@${n}`}function V(t){return(t.name??[t.first_name,t.last_name].filter(Boolean).join(" ")).trim()}function q(t,e){return t.business_name??t.merchant_name??t.name??e??"PayIsland merchant"}function b(t){if(t)try{let e=new URL(t);if(e.protocol==="https:"||e.protocol==="http:"&&xe(e.hostname))return e.toString()}catch{return}}function xe(t){return t==="localhost"||t==="127.0.0.1"||t==="::1"}function M(t){let e=t,r=[t.bank_transfer,t.bankTransfer,e.bank_account,e.bankAccount,e.virtual_account,e.virtualAccount,e.transfer_account,e.transferAccount,e.account_details,e.accountDetails].find(B);return r||(B(t)?t:void 0)}function B(t){if(!t||typeof t!="object")return!1;let e=t;return!!(e.account_number??e.accountNumber??e.account_no??e.accountNo??(e.account&&typeof e.account=="object"&&(e.account.number??e.account.account_number??e.account.accountNumber)))}function X(t,e){return t===404?new p({code:"reference_not_found",message:"This checkout link is invalid or has expired.",status:t,requestId:e}):t>=400&&t<500?new p({code:"validation_error",message:"We could not continue with this checkout. Please confirm the payment details.",status:t,requestId:e}):new p({code:"network_error",message:"We could not reach PayIsland. Please check your connection and try again.",status:t,requestId:e})}var S=class{constructor(e){this.baseUrl=e}setCheckoutToken(e){this.checkoutToken=e}async bootstrap(e){return this.request(`/api/v1/transactions/gateway/pay/keys/${e}`,{method:"GET"})}async submitPayment(e,n){return this.request(`/api/v1/transactions/gateway/pay/${e}`,{method:"POST",body:JSON.stringify(n)})}async verifyOtp(e,n){return this.request(`/api/v1/transactions/gateway/otp/verify/${e}`,{method:"POST",body:JSON.stringify(n)})}async verify(e){return this.request(`/api/v1/transactions/gateway/transaction/verify/${e}`,{method:"POST",body:JSON.stringify({})})}async verifyBankTransfer(e){return this.request(`/api/v1/transactions/gateway/transaction/verify-bank-transfer/${e}`,{method:"GET"})}async updatePaymentChannel(e,n){return this.request(`/api/v1/transactions/customer/update-payment-channel/${e}`,{method:"POST",body:JSON.stringify({channel:n})})}async request(e,n){let r;try{r=await fetch(`${this.baseUrl}${e}`,{...n,headers:{Accept:"application/json",...n.method!=="GET"?{"Content-Type":"application/json"}:{},...this.checkoutToken?{"x-checkout-token":this.checkoutToken}:{},...n.headers},credentials:"omit"})}catch{throw new p({code:"network_error",message:"We could not reach PayIsland. Please check your connection and try again."})}let o=K(r.headers);if(!r.ok)throw X(r.status,o);return{data:await we(r)??{},checkoutToken:r.headers.get("x-checkout-token")??void 0,requestId:o}}};async function we(t){if((t.headers.get("content-type")??"").includes("application/json"))return await t.json()}function H(t){let e=t?.account,n=e&&typeof e=="object"?e:void 0,r=t?.bank&&typeof t.bank=="object"?t.bank:void 0,o=t?.account_number??t?.accountNumber??t?.account_no??t?.accountNo??m(n?.number)??m(n?.account_number)??m(n?.accountNumber);return o?{accountNumber:o,accountName:t?.account_name??t?.accountName??t?.name??m(n?.name)??m(n?.account_name)??m(n?.accountName)??"PayIsland checkout",bankName:t?.bank_name??t?.bankName??(typeof t?.bank=="string"?t.bank:void 0)??r?.name??r?.bank_name??r?.bankName??"Bank transfer",expiresAt:t?.expires_at??t?.expiresAt??t?.expiry_date??t?.expiryDate}:null}function m(t){return typeof t=="string"&&t.trim()?t:void 0}function Y(){return"Card payments are not available in this checkout yet."}function Z(t){let e=b(t);return e?(window.open(e,"_blank","noopener,noreferrer"),!0):!1}var T="data:image/png;base64,";var L="data:image/png;base64,";var Q=`
:host {
  all: initial;
  --pi-primary: #074761;
  --pi-primary-strong: #05364a;
  --pi-primary-soft: #e7f2f6;
  --pi-accent: #8aa0ad;
  --pi-text: #111827;
  --pi-muted: #607080;
  --pi-border: #d8e1e8;
  --pi-border-strong: #b9c7d1;
  --pi-surface: #ffffff;
  --pi-soft: #f6f9fb;
  --pi-raised: #fbfdfe;
  --pi-danger: #b42318;
  --pi-danger-soft: #fff1f0;
  --pi-success: #067647;
  --pi-success-soft: #e8f8ef;
  --pi-warning: #9a6700;
  --pi-warning-soft: #fff7df;
  --pi-focus: rgba(7, 71, 97, 0.28);
  font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
}
* {
  box-sizing: border-box;
}
button,
input {
  font: inherit;
}
button {
  -webkit-tap-highlight-color: transparent;
}
button:focus-visible {
  outline: 3px solid var(--pi-focus);
  outline-offset: 2px;
}
.pi-overlay {
  position: fixed;
  inset: 0;
  z-index: 2147483647;
  display: grid;
  place-items: center;
  padding: 20px;
  background: rgba(10, 20, 30, 0.58);
  backdrop-filter: blur(4px);
}
.pi-inline {
  width: 100%;
}
.pi-modal {
  width: min(100%, 480px);
  max-height: min(780px, calc(100vh - 40px));
  overflow: hidden;
  border: 1px solid rgba(216, 225, 232, 0.92);
  border-radius: 14px;
  background: var(--pi-surface);
  color: var(--pi-text);
  box-shadow: 0 28px 90px rgba(8, 18, 28, 0.28);
}
.pi-inline .pi-modal {
  max-height: none;
  box-shadow: 0 10px 32px rgba(8, 18, 28, 0.08);
}
.pi-header,
.pi-body,
.pi-footer {
  padding: 16px 18px;
}
.pi-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 14px;
  border-bottom: 1px solid var(--pi-border);
  background: linear-gradient(180deg, #ffffff 0%, #f8fbfd 100%);
}
.pi-header-main,
.pi-brand {
  display: flex;
  align-items: center;
  min-width: 0;
  gap: 12px;
}
.pi-logo-frame {
  display: grid;
  width: 42px;
  height: 42px;
  flex: 0 0 auto;
  place-items: center;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: #fff;
  box-shadow: 0 8px 18px rgba(7, 71, 97, 0.08);
}
.pi-logo,
.pi-logo-fallback {
  width: 28px;
  height: 28px;
  flex: 0 0 auto;
  object-fit: contain;
}
.pi-logo-fallback {
  display: grid;
  place-items: center;
  border-radius: 8px;
  background: var(--pi-primary);
  color: #fff;
  font-weight: 800;
}
.pi-heading {
  min-width: 0;
}
.pi-eyebrow,
.pi-label {
  display: block;
  margin: 0 0 3px;
  color: var(--pi-muted);
  font-size: 11px;
  font-weight: 800;
  letter-spacing: 0.04em;
  line-height: 1.2;
  text-transform: uppercase;
}
.pi-title {
  margin: 0;
  overflow-wrap: anywhere;
  color: var(--pi-text);
  font-size: 16px;
  font-weight: 800;
  line-height: 1.25;
}
.pi-subtitle {
  margin: 4px 0 0;
  color: var(--pi-muted);
  font-size: 13px;
  line-height: 1.35;
}
.pi-close {
  display: grid;
  width: 38px;
  height: 38px;
  flex: 0 0 auto;
  place-items: center;
  border: 1px solid transparent;
  border-radius: 999px;
  background: transparent;
  color: var(--pi-muted);
  cursor: pointer;
  font-size: 24px;
  line-height: 1;
}
.pi-close:hover,
.pi-close:focus-visible {
  border-color: var(--pi-border);
  background: #fff;
  color: var(--pi-text);
}
.pi-body {
  max-height: calc(100vh - 178px);
  overflow-y: auto;
  background: var(--pi-surface);
}
.pi-inline .pi-body {
  max-height: none;
}
.pi-stack {
  display: grid;
  gap: 16px;
}
.pi-summary {
  display: grid;
  overflow: hidden;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: var(--pi-raised);
}
.pi-summary-hero {
  display: flex;
  align-items: flex-end;
  justify-content: space-between;
  gap: 16px;
  padding: 14px;
  border-bottom: 1px solid var(--pi-border);
  background: linear-gradient(135deg, var(--pi-primary-soft), #ffffff 70%);
}
.pi-summary-hero strong {
  display: block;
  color: var(--pi-primary-strong);
  font-size: 26px;
  font-weight: 850;
  line-height: 1.05;
}
.pi-summary-hero p {
  max-width: 42%;
  margin: 0;
  color: var(--pi-muted);
  font-size: 13px;
  font-weight: 700;
  line-height: 1.35;
  text-align: right;
}
.pi-summary-grid {
  display: grid;
  gap: 0;
  padding: 4px 14px;
}
.pi-row {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 16px;
  min-height: 34px;
  border-bottom: 1px solid rgba(216, 225, 232, 0.72);
  color: var(--pi-muted);
  font-size: 14px;
}
.pi-row:last-child {
  border-bottom: 0;
}
.pi-row strong {
  min-width: 0;
  color: var(--pi-text);
  font-weight: 750;
  overflow-wrap: anywhere;
  text-align: right;
}
.pi-reference strong {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 700;
}
.pi-tabs {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(124px, 1fr));
  gap: 6px;
  padding: 5px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: var(--pi-soft);
}
.pi-tab {
  min-height: 40px;
  border: 1px solid transparent;
  border-radius: 9px;
  background: transparent;
  color: var(--pi-muted);
  cursor: pointer;
  font-size: 13px;
  font-weight: 800;
}
.pi-tab:hover:not([disabled]) {
  color: var(--pi-text);
  background: #fff;
}
.pi-tab[aria-selected="true"] {
  border-color: rgba(7, 71, 97, 0.18);
  background: var(--pi-primary);
  color: #fff;
  box-shadow: 0 8px 18px rgba(7, 71, 97, 0.18);
}
.pi-tab[disabled] {
  cursor: not-allowed;
  opacity: 0.48;
}
.pi-panel {
  display: grid;
  gap: 12px;
}
.pi-panel-title,
.pi-state-title {
  margin: 0;
  color: var(--pi-text);
  font-size: 16px;
  font-weight: 850;
  line-height: 1.25;
}
.pi-bank-box {
  display: grid;
  gap: 10px;
  padding: 14px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: #fff;
  box-shadow: 0 10px 24px rgba(8, 18, 28, 0.05);
}
.pi-bank-intro {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
.pi-bank-intro strong {
  color: var(--pi-primary-strong);
  font-size: 14px;
  font-weight: 850;
  text-align: right;
}
.pi-account {
  display: grid;
  gap: 8px;
  padding: 12px;
  border: 1px solid rgba(7, 71, 97, 0.16);
  border-radius: 12px;
  background: linear-gradient(180deg, var(--pi-primary-soft), #fff);
}
.pi-account > span {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 800;
  letter-spacing: 0.04em;
  text-transform: uppercase;
}
.pi-account-number {
  color: var(--pi-primary-strong);
  font-size: 28px;
  font-weight: 900;
  letter-spacing: 0;
  line-height: 1.1;
  overflow-wrap: anywhere;
}
.pi-bank-grid {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 10px;
}
.pi-detail {
  display: grid;
  gap: 5px;
  min-width: 0;
  padding: 12px;
  border: 1px solid var(--pi-border);
  border-radius: 10px;
  background: var(--pi-soft);
}
.pi-detail span {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 750;
}
.pi-detail strong {
  min-width: 0;
  color: var(--pi-text);
  font-size: 14px;
  font-weight: 800;
  overflow-wrap: anywhere;
}
.pi-copy,
.pi-primary,
.pi-secondary {
  min-height: 42px;
  border-radius: 10px;
  cursor: pointer;
  font-weight: 800;
}
.pi-copy {
  justify-self: start;
  border: 1px solid var(--pi-primary);
  background: #fff;
  color: var(--pi-primary);
  padding: 0 14px;
}
.pi-copy:hover,
.pi-secondary:hover {
  border-color: var(--pi-border-strong);
  background: var(--pi-soft);
}
.pi-primary {
  width: 100%;
  border: 0;
  background: var(--pi-primary);
  color: #fff;
  box-shadow: 0 10px 18px rgba(7, 71, 97, 0.18);
}
.pi-primary:hover,
.pi-primary:focus-visible {
  background: var(--pi-primary-strong);
}
.pi-primary:disabled,
.pi-secondary:disabled {
  cursor: not-allowed;
  opacity: 0.62;
  box-shadow: none;
}
.pi-secondary {
  border: 1px solid var(--pi-border);
  background: #fff;
  color: var(--pi-text);
  padding: 0 14px;
}
.pi-secondary:disabled {
  cursor: wait;
}
.pi-status {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
  padding: 11px 12px;
  border: 1px solid rgba(154, 103, 0, 0.2);
  border-radius: 10px;
  background: var(--pi-warning-soft);
  color: #6f4a00;
  font-size: 13px;
}
.pi-status strong {
  color: #5f3f00;
  font-weight: 850;
  text-align: right;
  text-transform: capitalize;
}
.pi-bank-actions {
  display: grid;
}
.pi-refresh {
  width: 100%;
}
.pi-redirect-card {
  display: grid;
  gap: 14px;
  padding: 16px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: linear-gradient(180deg, #ffffff, var(--pi-soft));
}
.pi-redirect-icon {
  display: grid;
  width: 42px;
  height: 42px;
  place-items: center;
  border-radius: 999px;
  background: var(--pi-primary-soft);
  color: var(--pi-primary);
  font-size: 20px;
  font-weight: 900;
}
.pi-state {
  display: grid;
  justify-items: center;
  gap: 10px;
  padding: 34px 14px;
  text-align: center;
}
.pi-state-logo {
  width: 36px;
  height: 36px;
  object-fit: contain;
}
.pi-spinner {
  width: 34px;
  height: 34px;
  border: 3px solid var(--pi-border);
  border-top-color: var(--pi-primary);
  border-radius: 50%;
  animation: pi-spin 0.8s linear infinite;
}
.pi-badge {
  display: inline-grid;
  place-items: center;
  min-width: 40px;
  height: 40px;
  border-radius: 999px;
  font-weight: 900;
}
.pi-badge-success {
  background: var(--pi-success-soft);
  color: var(--pi-success);
}
.pi-badge-error {
  background: var(--pi-danger-soft);
  color: var(--pi-danger);
}
.pi-badge-warning {
  background: var(--pi-warning-soft);
  color: var(--pi-warning);
}
.pi-message {
  margin: 0;
  color: var(--pi-muted);
  font-size: 14px;
  line-height: 1.5;
}
.pi-error {
  padding: 12px;
  border: 1px solid #fecdca;
  border-radius: 10px;
  background: #fffbfa;
  color: var(--pi-danger);
  font-size: 14px;
}
.pi-unavailable {
  padding: 14px;
  border: 1px dashed var(--pi-border-strong);
  border-radius: 10px;
  background: var(--pi-soft);
  color: var(--pi-muted);
  font-size: 14px;
}
.pi-footer {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
  border-top: 1px solid var(--pi-border);
  background: var(--pi-raised);
}
.pi-secured {
  display: inline-flex;
  align-items: center;
  min-width: 0;
  gap: 7px;
  padding: 7px 10px;
  border-radius: 999px;
  background: var(--pi-primary-strong);
  color: rgba(255, 255, 255, 0.78);
  font-size: 12px;
  line-height: 1;
}
.pi-secured strong {
  color: #fff;
  font-weight: 850;
}
.pi-secured-logo {
  width: 15px;
  height: 20px;
  object-fit: contain;
}
.pi-sr-only {
  position: absolute;
  width: 1px;
  height: 1px;
  padding: 0;
  overflow: hidden;
  clip: rect(0, 0, 0, 0);
  white-space: nowrap;
  border: 0;
}
@keyframes pi-spin {
  to {
    transform: rotate(360deg);
  }
}
@media (max-width: 520px) {
  .pi-overlay {
    align-items: end;
    padding: 10px;
  }
  .pi-modal {
    width: 100%;
    max-height: calc(100vh - 20px);
    border-radius: 14px 14px 10px 10px;
  }
  .pi-header,
  .pi-body,
  .pi-footer {
    padding: 15px;
  }
  .pi-header {
    gap: 10px;
  }
  .pi-logo-frame {
    width: 38px;
    height: 38px;
    border-radius: 10px;
  }
  .pi-title {
    font-size: 15px;
  }
  .pi-summary-hero {
    align-items: flex-start;
    flex-direction: column;
    gap: 8px;
  }
  .pi-summary-hero p {
    max-width: none;
    text-align: left;
  }
  .pi-summary-hero strong {
    font-size: 25px;
  }
  .pi-bank-intro,
  .pi-status {
    align-items: flex-start;
    flex-direction: column;
    gap: 6px;
  }
  .pi-bank-intro strong,
  .pi-status strong {
    text-align: left;
  }
  .pi-bank-grid {
    grid-template-columns: 1fr;
  }
  .pi-account-number {
    font-size: 26px;
  }
  .pi-tabs {
    grid-template-columns: 1fr;
  }
  .pi-footer {
    align-items: stretch;
    flex-direction: column;
  }
  .pi-secured {
    justify-content: center;
  }
}
`;var ee=["a[href]","button:not([disabled])","input:not([disabled])","select:not([disabled])","textarea:not([disabled])",'[tabindex]:not([tabindex="-1"])'].join(","),v=class{constructor(e){this.inline=!1;this.refreshingStatus=!1;this.options=e,this.host=document.createElement("div"),this.host.setAttribute("data-payisland-checkout",""),this.root=this.host.attachShadow({mode:"open"})}mount(){this.previousActiveElement=document.activeElement;let e=this.resolveContainer();this.inline=!!e,(e??document.body).appendChild(this.host),this.renderShell(),this.focusFirst()}destroy(){this.stopCountdown(),this.host.remove(),this.previousActiveElement instanceof HTMLElement&&this.previousActiveElement.focus({preventScroll:!0})}renderLoading(){this.setBody(`
      <div class="pi-state pi-state-loading" role="status" aria-live="polite">
        <img class="pi-state-logo" src="${f(T)}" alt="" />
        <div class="pi-spinner" aria-hidden="true"></div>
        <h3 class="pi-state-title">Preparing checkout</h3>
        <p class="pi-message">Preparing secure checkout...</p>
      </div>
    `)}getSelectedChannel(){return this.selectedChannel}renderError(e,n=!0){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        <div class="pi-badge pi-badge-error" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Checkout unavailable</h3>
        <p class="pi-message">${c(e.message)}</p>
        ${n?'<button class="pi-secondary" type="button" data-action="retry">Retry</button>':""}
      </div>
    `)}renderCheckout(e,n,r){let o=this.filterChannels(F(e),n,e);return o.length===0?(this.selectedChannel=void 0,this.renderError({code:"no_available_channels",message:"No available payment channels for this checkout."},!1),!1):(this.selectedChannel=this.selectedChannel&&o.includes(this.selectedChannel)?this.selectedChannel:o.find(a=>this.isSupported(a,e))??o[0],this.setBody(`
      <div class="pi-stack">
        ${this.renderSummary(e)}
        ${this.renderTabs(o,e)}
        <div class="pi-panel">${this.renderChannel(e,this.selectedChannel,r)}</div>
      </div>
    `),this.startCountdown(e),!0)}renderPending(e){this.setBody(`
      <div class="pi-state pi-state-pending" role="status" aria-live="polite">
        <div class="pi-spinner" aria-hidden="true"></div>
        <h3 class="pi-state-title">Confirming payment</h3>
        <p class="pi-message">Payment is pending. We are checking for confirmation.</p>
        ${e?.status?`<p class="pi-subtitle">Status: ${c(e.status)}</p>`:""}
      </div>
    `)}renderSuccess(){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-success" role="status" aria-live="polite">
        <div class="pi-badge pi-badge-success" aria-hidden="true">\u2713</div>
        <h3 class="pi-state-title">Payment successful</h3>
        <p class="pi-message">Payment successful.</p>
      </div>
    `)}renderFailure(e){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        <div class="pi-badge pi-badge-error" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Payment failed</h3>
        <p class="pi-message">${c(e)}</p>
      </div>
    `)}renderExpired(){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-expired" role="alert">
        <div class="pi-badge pi-badge-warning" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Checkout expired</h3>
        <p class="pi-message">This checkout has expired. Please start a new payment.</p>
      </div>
    `)}setStatusRefreshing(e){this.refreshingStatus=e;let n=this.root.querySelector('[data-action="refresh-status"]');n&&(n.disabled=e,n.textContent=e?"Checking...":"Refresh status")}setChannelLoading(e,n){this.loadingChannel=n?e:void 0}renderShell(){let e=this.options.theme.merchantName??"PayIsland Checkout",n=this.options.theme.logoUrl??T,r=this.inline?"pi-inline":"pi-overlay";this.root.innerHTML=`
      <style>${Q}</style>
      <div class="${r}" data-shell>
        <section class="pi-modal" role="dialog" aria-modal="${this.inline?"false":"true"}" aria-labelledby="pi-title">
          <header class="pi-header">
            <div class="pi-header-main">
              <div class="pi-logo-frame">
                <img class="pi-logo" src="${f(n)}" alt="" />
              </div>
              <div class="pi-heading">
                <p class="pi-eyebrow">Secure checkout</p>
                <h2 class="pi-title" id="pi-title">${c(e)}</h2>
                <p class="pi-subtitle">Protected by PayIsland</p>
              </div>
            </div>
            <button class="pi-close" type="button" aria-label="Close checkout" data-action="close">
              <span aria-hidden="true">\xD7</span>
            </button>
          </header>
          <main class="pi-body" data-body></main>
          <footer class="pi-footer">
            <div class="pi-secured">
              <span>Secured by</span>
              <img class="pi-secured-logo" src="${f(L)}" alt="PayIsland" />
              <strong>PayIsland</strong>
            </div>
            <button class="pi-secondary" type="button" data-action="close">Cancel</button>
          </footer>
        </section>
      </div>
    `,this.root.querySelector("[data-shell]")?.addEventListener("click",o=>{!this.inline&&o.target===o.currentTarget&&this.options.onClose("user")}),this.root.addEventListener("click",o=>this.handleClick(o)),this.root.addEventListener("keydown",o=>this.handleKeydown(o)),this.options.theme.primaryColor&&this.host.style.setProperty("--pi-primary",this.options.theme.primaryColor)}setBody(e){let n=this.root.querySelector("[data-body]");n&&(n.innerHTML=e)}renderSummary(e){let n=l(e),r=J(e),o=G(e),a=n.currency??e.currency??"NGN",s=n.total_amount??n.totalAmount??n.amount_to_be_paid??n.amountToBePaid??e.total_amount??e.totalAmount??e.amount_to_be_paid??e.amountToBePaid,d=n.amount??e.amount,u=n.fee??n.transaction_fee??e.fee??e.transaction_fee,C=V(o),z=W(o.email),se=q(r,this.options.theme.merchantName),O=n.reference??e.reference??"",ce=y(s??te(d,u)??d,a);return`
      <section class="pi-summary" aria-label="Transaction summary">
        <div class="pi-summary-hero">
          <div>
            <span class="pi-label">Total due</span>
            <strong>${c(ce)}</strong>
          </div>
          <p>${c(se)}</p>
        </div>
        <div class="pi-summary-grid">
          <div class="pi-row"><span>Amount</span><strong>${c(y(d,a))}</strong></div>
          <div class="pi-row"><span>Fee</span><strong>${c(y(u,a))}</strong></div>
          ${C?`<div class="pi-row"><span>Customer</span><strong>${c(C)}</strong></div>`:""}
          ${z?`<div class="pi-row"><span>Email</span><strong>${c(z)}</strong></div>`:""}
          ${O?`<div class="pi-row pi-reference"><span>Reference</span><strong>${c(O)}</strong></div>`:""}
        </div>
      </section>
    `}renderTabs(e,n){return`
      <div class="pi-tabs" role="tablist" aria-label="Payment channels">
        ${e.map(r=>{let o=this.isSupported(r,n);return`
              <button
                class="pi-tab"
                type="button"
                role="tab"
                aria-selected="${r===this.selectedChannel?"true":"false"}"
                data-channel="${f(r)}"
                ${o?"":"disabled"}
              >
                ${c(I(r))}
              </button>
            `}).join("")}
      </div>
    `}renderChannel(e,n,r){if(n&&this.loadingChannel===n)return`
        <div class="pi-state pi-state-loading" role="status" aria-live="polite">
          <div class="pi-spinner" aria-hidden="true"></div>
          <h3 class="pi-state-title">Preparing ${c(I(n))}</h3>
          <p class="pi-message">Setting up this payment method...</p>
        </div>
      `;if(n==="bank-transfer"){let o=H(h(e)),a=l(e),s=a.currency??e.currency??"NGN",d=a.amount??e.amount,u=a.fee??a.transaction_fee??e.fee??e.transaction_fee,C=a.total_amount??a.totalAmount??a.amount_to_be_paid??a.amountToBePaid??e.total_amount??e.totalAmount??e.amount_to_be_paid??e.amountToBePaid??te(d,u)??d;return o?`
        <section class="pi-bank-box" aria-label="Bank transfer details">
          <div class="pi-bank-intro">
            <span class="pi-label">Bank transfer</span>
            <strong>Transfer exactly ${c(y(C,s))}</strong>
          </div>
          <div class="pi-account">
            <span>Account number</span>
            <strong class="pi-account-number">${c(o.accountNumber)}</strong>
            <button class="pi-copy" type="button" data-copy="${f(o.accountNumber)}">Copy number</button>
          </div>
          <div class="pi-bank-grid">
            <div class="pi-detail"><span>Bank</span><strong>${c(o.bankName)}</strong></div>
            <div class="pi-detail"><span>Account name</span><strong>${c(o.accountName)}</strong></div>
          </div>
          ${o.expiresAt?`<p class="pi-subtitle" data-countdown="${f(o.expiresAt)}"></p>`:""}
          <div class="pi-status" role="status" aria-live="polite">
            <span>Current status</span>
            <strong>${c(Ce(r))}</strong>
          </div>
          <div class="pi-bank-actions">
            <button class="pi-secondary pi-refresh" type="button" data-action="refresh-status" ${this.refreshingStatus?"disabled":""}>
              ${this.refreshingStatus?"Checking...":"Refresh status"}
            </button>
          </div>
          <p class="pi-message">Keep this checkout open while we confirm your payment.</p>
        </section>
      `:'<div class="pi-unavailable">Bank transfer details are not available yet.</div>'}if(n==="redirect"||b(_(e))){let o=b(_(e));return`
        <section class="pi-redirect-card">
          <div class="pi-redirect-icon" aria-hidden="true">\u2197</div>
          <div>
            <h3 class="pi-panel-title">Continue securely</h3>
            <p class="pi-message">You will continue to a secure PayIsland payment page. Keep this checkout open while we confirm the payment.</p>
          </div>
          <button class="pi-primary" type="button" data-redirect="${f(o??"")}" ${o?"":"disabled"}>
            Continue to payment
          </button>
        </section>
      `}return n==="card"?`<div class="pi-unavailable">${c(Y())}</div>`:`<div class="pi-unavailable">${c(I(n??"This channel"))} is not available in this checkout yet.</div>`}startCountdown(e){this.stopCountdown();let n=H(h(e));if(!n?.expiresAt)return;let r=new Date(n.expiresAt).getTime(),o=this.root.querySelector("[data-countdown]");if(!Number.isFinite(r)||!o)return;let a=()=>{let s=Math.max(0,r-Date.now()),d=Math.floor(s/6e4),u=Math.floor(s%6e4/1e3);o.textContent=s>0?`Transfer account expires in ${d}:${u.toString().padStart(2,"0")}`:"Transfer account has expired."};a(),this.countdownTimer=window.setInterval(a,1e3)}stopCountdown(){this.countdownTimer&&(window.clearInterval(this.countdownTimer),this.countdownTimer=void 0)}filterChannels(e,n,r){let o=[...new Set(e)];return n?.length?o.filter(a=>n.includes(a)&&(r?this.isSupported(a,r):!0)):o.length>0?o:["bank-transfer","redirect","card"]}isSupported(e,n){return e==="bank-transfer"||e==="redirect"||e==="card"||e==="mono"}handleClick(e){let n=e.target;if(!(n instanceof HTMLElement))return;if(n.closest('[data-action="close"]')){this.options.onClose("user");return}if(n.closest('[data-action="retry"]')){this.options.onRetry();return}if(n.closest('[data-action="refresh-status"]')){this.selectedChannel&&this.options.onPaymentStarted(this.selectedChannel),this.options.onRefreshStatus();return}let s=n.closest("[data-channel]")?.dataset.channel;if(s){this.selectedChannel=s,this.options.onChannelSelected(s);return}let d=n.closest("[data-copy]")?.dataset.copy;if(d){navigator.clipboard?.writeText(d),n.textContent="Copied";return}let u=n.closest("[data-redirect]")?.dataset.redirect;u&&(Z(u),this.options.onPaymentStarted(this.selectedChannel??"redirect"))}handleKeydown(e){if(e.key==="Escape"){this.options.onClose("user");return}if(e.key!=="Tab"||this.inline)return;let n=Array.from(this.root.querySelectorAll(ee));if(n.length===0)return;let r=n[0],o=n[n.length-1],a=this.root.activeElement;e.shiftKey&&a===r?(e.preventDefault(),o.focus()):!e.shiftKey&&a===o&&(e.preventDefault(),r.focus())}focusFirst(){window.setTimeout(()=>{this.root.querySelector(ee)?.focus()},0)}resolveContainer(){let e=this.options.container;if(e)return e instanceof HTMLElement?e:document.querySelector(e)??void 0}};function I(t){return{"bank-transfer":"Bank transfer",redirect:"Redirect",card:"Card",mono:"Mono"}[t]??String(t)}function Ce(t){if(!t)return"Pending confirmation";let e=String(t).trim();return e?e.replace(/[-_]+/g," "):"Pending confirmation"}function te(t,e){if(t==null||t===""||e==null||e==="")return;let n=Number(t),r=Number(e);if(!(!Number.isFinite(n)||!Number.isFinite(r)))return Math.round((n+r)*100)/100}function c(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function f(t){return c(t)}var x=class{constructor(){this.stopped=!0}start(e,n,r=0){this.stop(),this.stopped=!1;let o=async()=>{if(!this.stopped)try{let a=await e();if(this.stopped||(n(a),this.stopped))return;this.timer=window.setTimeout(o,g(a))}catch{this.stopped||(this.timer=window.setTimeout(o,5e3))}};this.timer=window.setTimeout(o,r)}stop(){this.stopped=!0,this.timer&&(window.clearTimeout(this.timer),this.timer=void 0)}};var _e={idle:{LOAD:"loading",CLOSE:"closed"},loading:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},ready:{PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},pending:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},success:{CLOSE:"closed"},failed:{CLOSE:"closed"},expired:{CLOSE:"closed"},closed:{}},w=class{constructor(e){this.context=e}send(e){let n=_e[this.context.status][e.type];return n&&(this.context.status=n),this.context.status}isTerminal(){return this.context.status==="success"||this.context.status==="failed"||this.context.status==="expired"||this.context.status==="closed"}};var i;function oe(t){k(!1);let e;try{e=j(t?.reference)}catch(s){let d=$(s);t?.onError?.(d),re(t,d);return}let n;try{n=new S(A(t.__apiBaseUrl))}catch(s){let d=$(s);t.onError?.(d),re(t,d);return}let r=new w({reference:e,theme:t.theme??{},status:"idle"}),o=new x,a=new v({container:t.container,theme:t.theme??{},onClose:s=>k(s==="user"),onRetry:()=>{ne()},onChannelSelected:s=>{i?.bootstrap&&(i.machine.context.selectedChannel=s,i.modal.renderCheckout(i.bootstrap,i.options.channels,i.machine.context.status),Se(s))},onPaymentStarted:()=>Te(),onRefreshStatus:()=>{$e()}});i={options:t,reference:e,api:n,modal:a,poller:o,machine:r,successCalled:!1,errorCalled:!1,closeCalled:!1},a.mount(),ne()}function k(t=!0){if(!i)return;let e=i;i=void 0,e.poller.stop(),e.machine.send({type:"CLOSE"}),e.modal.destroy(),e.api?.setCheckoutToken(void 0),e.bootstrap=void 0,t&&!e.closeCalled&&(e.closeCalled=!0,e.options.onClose?.())}async function ne(){let t=i;if(!(!t||!t.api)){t.poller.stop(),t.machine.send({type:"LOAD"}),t.modal.renderLoading();try{let e=await t.api.bootstrap(t.reference);if(i!==t)return;if(t.bootstrap=e.data,t.machine.context.bootstrap=e.data,t.machine.context.checkoutToken=e.checkoutToken,t.api.setCheckoutToken(e.checkoutToken),!t.modal.renderCheckout(e.data,t.options.channels)){t.machine.send({type:"FAIL"}),P({code:"no_available_channels",message:"No available payment channels for this checkout."});return}N(e.data,!1)}catch(e){if(i!==t)return;let n=$(e);t.machine.context.error=n,t.machine.send({type:"FAIL"}),t.modal.renderError(n),P(n)}}}async function Se(t){let e=i;if(!(!e||!e.api||!e.bootstrap)){e.modal.setChannelLoading(t,!0),e.modal.renderCheckout(e.bootstrap,e.options.channels,e.machine.context.status);try{let n=await e.api.updatePaymentChannel(e.reference,Re(t));if(i!==e)return;Ee(n.data),e.bootstrap=He(e.bootstrap,n.data),e.machine.context.bootstrap=e.bootstrap}catch(n){let r=$(n);P(r)}finally{i===e&&e.bootstrap&&(e.modal.setChannelLoading(t,!1),e.modal.renderCheckout(e.bootstrap,e.options.channels,e.machine.context.status))}}}function Te(){let t=i;!t||!t.bootstrap||t.machine.isTerminal()||D(g(t.bootstrap))}function D(t){let e=i;!e||!e.api||e.poller.start(()=>ae(e),n=>N(n,!0),t)}async function $e(){let t=i;if(!(!t||!t.api)){t.poller.stop(),t.modal.setStatusRefreshing(!0);try{let e=await ae(t);if(i!==t)return;N(e,!0),t.machine.isTerminal()||D(g(e))}catch{i===t&&!t.machine.isTerminal()&&D(5e3)}finally{i===t&&t.modal.setStatusRefreshing(!1)}}}async function ae(t){if(!t.api)return{};if(t.modal.getSelectedChannel()==="bank-transfer"&&h(t.bootstrap))try{return(await t.api.verifyBankTransfer(t.reference)).data}catch{return(await t.api.verify(t.reference)).data}return(await t.api.verify(t.reference)).data}function N(t,e){let n=i;if(!n)return;let r=U(E(t)),o=l(t);if(r==="success"){n.poller.stop(),n.machine.send({type:"SUCCESS"}),n.modal.renderSuccess(),n.successCalled||(n.successCalled=!0,n.options.onSuccess?.(o));return}if(r==="expired"){n.poller.stop(),n.machine.send({type:"EXPIRE"}),n.modal.renderExpired(),P({code:"payment_expired",message:"This checkout has expired. Please start a new payment."});return}if(r==="failed"){n.poller.stop(),n.machine.send({type:"FAIL"}),n.modal.renderFailure("This payment could not be completed."),P({code:"payment_failed",message:"This payment could not be completed."});return}e?(n.machine.send({type:"PENDING"}),n.bootstrap?n.modal.renderCheckout(n.bootstrap,n.options.channels,E(t)??r):n.modal.renderPending(t),n.options.onPending?.(t)):n.machine.send({type:"READY"})}function P(t){let e=i;!e||e.errorCalled||(e.errorCalled=!0,e.options.onError?.(t))}function Re(t){return t==="redirect"?"card":t}function Ee(t){let e=t;if(e.status===!1)throw new p({code:"payment_channel_update_failed",message:typeof e.message=="string"&&e.message.trim()?e.message:"We could not set up this payment method."})}function He(t,e){let n=l(t),r=l(e),o=n.fee??n.transaction_fee,s=r.amount!==void 0&&r.amount_to_be_paid===void 0&&r.amountToBePaid===void 0&&r.transaction_fee===void 0&&r.fee===void 0&&o!==void 0?{...r,amount:n.amount,amount_to_be_paid:r.amount}:r;return{...t,data:{...n,...s}}}function re(t,e){let n=new v({container:t?.container,theme:t?.theme??{},onClose:o=>k(o==="user"),onRetry:()=>{},onChannelSelected:()=>{},onPaymentStarted:()=>{},onRefreshStatus:()=>{}}),r=new w({reference:"",theme:t?.theme??{},status:"failed"});i={options:t??{reference:""},reference:"",modal:n,poller:new x,machine:r,successCalled:!1,errorCalled:!0,closeCalled:!1},n.mount(),n.renderError(e,!1)}function $(t){return t instanceof p?t.toPayload():{code:"checkout_error",message:"We could not continue with this checkout."}}var ie={open:oe,close:k};typeof window<"u"&&(window.PayIslandCheckout=ie);var Le=ie;return he(Ie);})();
