...
`;var ee=["a[href]","button:not([disabled])","input:not([disabled])","select:not([disabled])","textarea:not([disabled])",'[tabindex]:not([tabindex="-1"])'].join(","),k=class{constructor(e){this.inline=!1;this.refreshingStatus=!1;this.options=e,this.host=document.createElement("div"),this.host.setAttribute("data-payisland-checkout",""),this.root=this.host.attachShadow({mode:"open"})}mount(){this.previousActiveElement=document.activeElement;let e=this.resolveContainer();this.inline=!!e,(e??document.body).appendChild(this.host),this.renderShell(),this.focusFirst()}destroy(){this.stopCountdown(),this.host.remove(),this.previousActiveElement instanceof HTMLElement&&this.previousActiveElement.focus({preventScroll:!0})}renderLoading(){this.setBody(`
      <div class="pi-state pi-state-loading" role="status" aria-live="polite">
        <img class="pi-state-logo" src="${m(T)}" alt="" />
    ...
    `)}getSelectedChannel(){return this.selectedChannel}renderError(e,n=!0){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        ...
        <p class="pi-message">${c(e.message)}</p>
        ${n?'<button class="pi-secondary" type="button" data-action="retry">Retry</button>':""}
      </div>
    `)}renderCheckout(e,n,r){let a=this.filterChannels(J(e),n,e);return a.length===0?(this.selectedChannel=void 0,this.renderError({code:"no_available_channels",message:"No available payment channels for this checkout."},!1),!1):(this.selectedChannel=this.selectedChannel&&a.includes(this.selectedChannel)?this.selectedChannel:a.find(i=>this.isSupported(i,e))??a[0],this.setBody(`
      <div class="pi-stack">
        ${this.renderSummary(e)}
        ${this.renderTabs(a,e)}
        <div class="pi-panel">${this.renderChannel(e,this.selectedChannel,r)}</div>
      </div>
    `),this.startCountdown(e),!0)}renderPending(e){this.setBody(`
      <div class="pi-state pi-state-pending" role="status" aria-live="polite">
        ...
        ${e?.status?`<p class="pi-subtitle">Status: ${c(e.status)}</p>`:""}
      </div>
    `)}renderSuccess(){this.stopCountdown(),this.setBody(`
    ...
    `)}renderFailure(e){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        <div class="pi-badge pi-badge-error" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Payment failed</h3>
        <p class="pi-message">${c(e)}</p>
      </div>
    `)}renderExpired(){this.stopCountdown(),this.setBody(`
    ...
    `)}setStatusRefreshing(e){this.refreshingStatus=e;let n=this.root.querySelector('[data-action="refresh-status"]');n&&(n.disabled=e,n.textContent=e?"Checking...":"Refresh status")}setChannelLoading(e,n){this.loadingChannel=n?e:void 0}renderShell(){let e=this.options.theme.merchantName??"PayIsland Checkout",n=this.options.theme.logoUrl??T,r=this.inline?"pi-inline":"pi-overlay";this.root.innerHTML=`
      <style>${Q}</style>
      <div class="${r}" data-shell>
        <section class="pi-modal" role="dialog" aria-modal="${this.inline?"false":"true"}" aria-labelledby="pi-title">
          <header class="pi-header">
            <div class="pi-header-main">
              <div class="pi-logo-frame">
                <img class="pi-logo" src="${m(n)}" alt="" />
              </div>
              <div class="pi-heading">
                <p class="pi-eyebrow">Secure checkout</p>
                <h2 class="pi-title" id="pi-title">${c(e)}</h2>
          ...
          <footer class="pi-footer">
            <div class="pi-secured">
              ...
              <img class="pi-secured-logo" src="${m(I)}" alt="PayIsland" />
    ...
    `,this.root.querySelector("[data-shell]")?.addEventListener("click",a=>{!this.inline&&a.target===a.currentTarget&&this.options.onClose("user")}),this.root.addEventListener("click",a=>this.handleClick(a)),this.root.addEventListener("keydown",a=>this.handleKeydown(a)),this.options.theme.primaryColor&&this.host.style.setProperty("--pi-primary",this.options.theme.primaryColor)}setBody(e){let n=this.root.querySelector("[data-body]");n&&(n.innerHTML=e)}renderSummary(e){let n=p(e),r=G(e),a=W(e),i=n.currency??e.currency??"NGN",s=n.total_amount??n.totalAmount??n.amount_to_be_paid??n.amountToBePaid??e.total_amount??e.totalAmount??e.amount_to_be_paid??e.amountToBePaid,d=n.amount??e.amount,u=n.fee??n.transaction_fee??e.fee??e.transaction_fee,f=q(a),M=V(a.email),oe=X(r,this.options.theme.merchantName),O=n.reference??e.reference??"",se=w(s??te(d,u)??d,i);return`
      <section class="pi-summary" aria-label="Transaction summary">
        ...
        <div class="pi-summary-grid">
          <div class="pi-row"><span>Amount</span><strong>${c(w(d,i))}</strong></div>
          <div class="pi-row"><span>Fee</span><strong>${c(w(u,i))}</strong></div>
          ${f?`<div class="pi-row"><span>Customer</span><strong>${c(f)}</strong></div>`:""}
          ${M?`<div class="pi-row"><span>Email</span><strong>${c(M)}</strong></div>`:""}
          ${O?`<div class="pi-row pi-reference"><span>Reference</span><strong>${c(O)}</strong></div>`:""}
        </div>
      </section>
    `}renderTabs(e,n){return`
      <div class="pi-tabs" role="tablist" aria-label="Payment channels">
        ${e.map(r=>{let a=this.isSupported(r,n);return`
              <button
                class="pi-tab"
                type="button"
                role="tab"
                aria-selected="${r===this.selectedChannel?"true":"false"}"
                data-channel="${m(r)}"
                ${a?"":"disabled"}
              >
                ${c(D(r))}
    ...
    `}renderChannel(e,n,r){if(n&&this.loadingChannel===n)return`
        <div class="pi-state pi-state-loading" role="status" aria-live="polite">
          <div class="pi-spinner" aria-hidden="true"></div>
          <h3 class="pi-state-title">Preparing ${c(D(n))}</h3>
          <p class="pi-message">Setting up this payment method...</p>
        </div>
      `;if(n==="bank-transfer"){let a=L(g(e)),i=p(e),s=i.currency??e.currency??"NGN",d=i.amount??e.amount,u=i.fee??i.transaction_fee??e.fee??e.transaction_fee,f=i.total_amount??i.totalAmount??i.amount_to_be_paid??i.amountToBePaid??e.total_amount??e.totalAmount??e.amount_to_be_paid??e.amountToBePaid??te(d,u)??d;return a?`
        <section class="pi-bank-box" aria-label="Bank transfer details">
          <div class="pi-bank-intro">
            <span class="pi-label">Bank transfer</span>
            <strong>Transfer exactly ${c(w(f,s))}</strong>
          </div>
          <div class="pi-account">
            <span>Account number</span>
            <strong class="pi-account-number">${c(a.accountNumber)}</strong>
            <button class="pi-copy" type="button" data-copy="${m(a.accountNumber)}">Copy number</button>
          </div>
          <div class="pi-bank-grid">
            <div class="pi-detail"><span>Bank</span><strong>${c(a.bankName)}</strong></div>
            <div class="pi-detail"><span>Account name</span><strong>${c(a.accountName)}</strong></div>
          </div>
          ${a.expiresAt?`<p class="pi-subtitle" data-countdown="${m(a.expiresAt)}"></p>`:""}
          <div class="pi-status" role="status" aria-live="polite">
            <span>Current status</span>
            <strong>${c(Pe(r))}</strong>
          </div>
      ...
      `}return`<div class="pi-unavailable">${c(D(n??"This channel"))} is not available in this checkout yet.</div>`}startCountdown(e){this.stopCountdown();let n=L(g(e));if(!n?.expiresAt)return;let r=new Date(n.expiresAt).getTime(),a=this.root.querySelector("[data-countdown]");if(!Number.isFinite(r)||!a)return;let i=()=>{let s=Math.max(0,r-Date.now()),d=Math.floor(s/6e4),u=Math.floor(s%6e4/1e3);a.textContent=s>0?`Transfer account expires in ${d}:${u.toString().padStart(2,"0")}`:"Transfer account has expired."};i(),this.countdownTimer=window.setInterval(i,1e3)}stopCountdown(){this.countdownTimer&&(window.clearInterval(this.countdownTimer),this.countdownTimer=void 0)}filterChannels(e,n,r){let a=[...new Set(e.map(v))];if(n?.length){let i=n.map(v);return a.filter(s=>i.includes(s)&&(r?this.isSupported(s,r):!0))}return a.length>0?a:["card","bank-transfer"]}isSupported(e,n){return e==="bank-transfer"||e==="card"||e==="mono"}handleClick(e){let n=e.target;if(!(n instanceof HTMLElement))return;if(n.closest('[data-action="close"]')){this.options.onClose("user");return}if(n.closest('[data-action="retry"]')){this.options.onRetry();return}if(n.closest('[data-action="refresh-status"]')){this.selectedChannel&&this.options.onPaymentStarted(this.selectedChannel),this.options.onRefreshStatus();return}let s=n.closest('[data-action="initialize-channel"]')?.dataset.channelTarget;if(s){this.selectedChannel=s,this.options.onChannelSelected(s);return}let d=n.closest("[data-channel]")?.dataset.channel;if(d){let f=v(d);this.selectedChannel=f,this.options.onChannelSelected(f);return}let u=n.closest("[data-copy]")?.dataset.copy;if(u){navigator.clipboard?.writeText(u),n.textContent="Copied";return}}handleKeydown(e){if(e.key==="Escape"){this.options.onClose("user");return}if(e.key!=="Tab"||this.inline)return;let n=Array.from(this.root.querySelectorAll(ee));if(n.length===0)return;let r=n[0],a=n[n.length-1],i=this.root.activeElement;e.shiftKey&&i===r?(e.preventDefault(),a.focus()):!e.shiftKey&&i===a&&(e.preventDefault(),r.focus())}focusFirst(){window.setTimeout(()=>{this.root.querySelector(ee)?.focus()},0)}resolveContainer(){let e=this.options.container;if(e)return e instanceof HTMLElement?e:document.querySelector(e)??void 0}};function D(t){return{"bank-transfer":"Bank transfer",redirect:"Card",card:"Card",mono:"Mono"}[t]??String(t)}function Pe(t){if(!t)return"Pending confirmation";let e=String(t).trim();return e?e.replace(/[-_]+/g," "):"Pending confirmation"}function Ce(t){let e=p(t),n=typeof e.reference=="string"&&e.reference.trim()?e.reference.trim():typeof t.reference=="string"&&t.reference.trim()?t.reference.trim():void 0;if(!n)return;let r=H(t);if(!r)return;let a=Y(r),i=new URL("/sdk/card-frame",a??"https://checkout.payislands.com");return i.searchParams.set("reference",n),typeof window<"u"&&window.location.origin&&i.searchParams.set("parent_origin",window.location.origin),i.toString()}function te(t,e){if(t==null||t===""||e==null||e==="")return;let n=Number(t),r=Number(e);if(!(!Number.isFinite(n)||!Number.isFinite(r)))return Math.round((n+r)*100)/100}function c(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function m(t){return c(t)}var P=class{constructor(){this.stopped=!0}start(e,n,r=0){this.stop(),this.stopped=!1;let a=async()=>{if(!this.stopped)try{let i=await e();if(this.stopped||(n(i),this.stopped))return;this.timer=window.setTimeout(a,x(i))}catch{this.stopped||(this.timer=window.setTimeout(a,5e3))}};this.timer=window.setTimeout(a,r)}stop(){this.stopped=!0,this.timer&&(window.clearTimeout(this.timer),this.timer=void 0)}};var _e={idle:{LOAD:"loading",CLOSE:"closed"},loading:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},ready:{PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},pending:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},success:{CLOSE:"closed"},failed:{CLOSE:"closed"},expired:{CLOSE:"closed"},closed:{}},C=class{constructor(e){this.context=e}send(e){let n=_e[this.context.status][e.type];return n&&(this.context.status=n),this.context.status}isTerminal(){return this.context.status==="success"||this.context.status==="failed"||this.context.status==="expired"||this.context.status==="closed"}};var o;function ae(t){b(!1);let e;try{e=j(t?.reference)}catch(s){let d=_(s);t?.onError?.(d),re(t,d);return}let n;try{n=new S(F(t.__apiBaseUrl))}catch(s){let d=_(s);t.onError?.(d),re(t,d);return}let r=new C({reference:e,theme:t.theme??{},status:"idle"}),a=new P,i=new k({container:t.container,theme:t.theme??{},onClose:s=>b(s==="user"),onRetry:()=>{ne()},onChannelSelected:s=>{o?.bootstrap&&(o.machine.context.selectedChannel=s,o.modal.renderCheckout(o.bootstrap,o.options.channels,o.machine.context.status),Se(s))},onPaymentStarted:()=>Te(),onRefreshStatus:()=>{He()}});o={options:t,reference:e,api:n,modal:i,poller:a,machine:r,successCalled:!1,errorCalled:!1,closeCalled:!1},o.cardFrameMessageHandler=$e,typeof window<"u"&&window.addEventListener("message",o.cardFrameMessageHandler),i.mount(),ne()}function b(t=!0){if(!o)return;let e=o;o=void 0,e.poller.stop(),e.machine.send({type:"CLOSE"}),typeof window<"u"&&e.cardFrameMessageHandler&&window.removeEventListener("message",e.cardFrameMessageHandler),e.modal.destroy(),e.api?.setCheckoutToken(void 0),e.bootstrap=void 0,t&&!e.closeCalled&&(e.closeCalled=!0,e.options.onClose?.())}async function ne(){let t=o;if(!(!t||!t.api)){t.poller.stop(),t.machine.send({type:"LOAD"}),t.modal.renderLoading();try{let e=await t.api.bootstrap(t.reference);if(o!==t)return;if(t.bootstrap=e.data,t.machine.context.bootstrap=e.data,t.machine.context.checkoutToken=e.checkoutToken,t.api.setCheckoutToken(e.checkoutToken),!t.modal.renderCheckout(e.data,t.options.channels)){t.machine.send({type:"FAIL"}),h({code:"no_available_channels",message:"No available payment channels for this checkout."});return}$(e.data,!1)}catch(e){if(o!==t)return;let n=_(e);t.machine.context.error=n,t.machine.send({type:"FAIL"}),t.modal.renderError(n),h(n)}}}async function Se(t){let e=o;if(!(!e||!e.api||!e.bootstrap)){e.modal.setChannelLoading(t,!0),e.modal.renderCheckout(e.bootstrap,e.options.channels,e.machine.context.status);try{let n=await e.api.updatePaymentChannel(e.reference,Ie(t));if(o!==e)return;De(n.data),e.bootstrap=ze(e.bootstrap,n.data),e.machine.context.bootstrap=e.bootstrap}catch(n){let r=_(n);h(r)}finally{o===e&&e.bootstrap&&(e.modal.setChannelLoading(t,!1),e.modal.renderCheckout(e.bootstrap,e.options.channels,e.machine.context.status))}}}function Te(){let t=o;!t||!t.bootstrap||t.machine.isTerminal()||z(x(t.bootstrap))}function $e(t){let e=o;if(!e||!Ee(t.origin))return;let n=t.data;n?.source==="payisland-card-frame"&&(n.reference&&n.reference!==e.reference||(n.event==="card.started"&&(e.machine.context.selectedChannel="card",e.poller.stop()),n.event==="card.finished"&&(e.machine.context.selectedChannel="card",e.poller.stop(),Re(e)),n.event==="card.failed"&&h({code:"card_payment_failed",message:n.message||"This card payment could not be completed."})))}async function Re(t){try{let e=await N(t);if(o!==t)return;$(e,!0)}catch(e){if(o!==t)return;h(_(e))}}function Ee(t){try{let e=new URL(t);return e.protocol==="https:"&&(e.hostname==="checkout.payislands.com"||e.hostname==="app-demo.payislands.com"||e.hostname==="checkout-demo.payislands.com")||e.hostname==="localhost"||e.hostname==="127.0.0.1"}catch{return!1}}function z(t){let e=o;!e||!e.api||e.poller.start(()=>N(e),n=>$(n,!0),t)}async function He(){let t=o;if(!(!t||!t.api)){t.poller.stop(),t.modal.setStatusRefreshing(!0);try{let e=await N(t);if(o!==t)return;$(e,!0),t.machine.isTerminal()||z(x(e))}catch{o===t&&!t.machine.isTerminal()&&z(5e3)}finally{o===t&&t.modal.setStatusRefreshing(!1)}}}async function N(t){if(!t.api)return{};if(t.modal.getSelectedChannel()==="bank-transfer"&&g(t.bootstrap))try{return(await t.api.verifyBankTransfer(t.reference)).data}catch{return(await t.api.verify(t.reference)).data}return(await t.api.verify(t.reference)).data}function $(t,e){let n=o;if(!n)return;let r=K(E(t)),a=p(t);if(r==="success"){n.poller.stop(),n.machine.send({type:"SUCCESS"}),n.successCalled||(n.successCalled=!0,n.options.onSuccess?.(a)),b(!1),Le(n,a);return}if(r==="expired"){n.poller.stop(),n.machine.send({type:"EXPIRE"}),n.modal.renderExpired(),h({code:"payment_expired",message:"This checkout has expired. Please start a new payment."});return}if(r==="failed"){n.poller.stop(),n.machine.send({type:"FAIL"}),n.modal.renderFailure("This payment could not be completed."),h({code:"payment_failed",message:"This payment could not be completed."});return}e?(n.machine.send({type:"PENDING"}),n.modal.getSelectedChannel()==="card"||(n.bootstrap?n.modal.renderCheckout(n.bootstrap,n.options.channels,E(t)??r):n.modal.renderPending(t)),n.options.onPending?.(t)):n.machine.send({type:"READY"})}function h(t){let e=o;!e||e.errorCalled||(e.errorCalled=!0,e.options.onError?.(t))}function Le(t,e){if(typeof window>"u")return;let n=typeof e.reference=="string"&&e.reference.trim()?e.reference:t.reference,r=new URL("/payment-status",t.options.__receiptBaseUrl??"https://checkout.payislands.com");r.searchParams.set("reference",n),r.searchParams.set("status","success"),t.machine.context.checkoutToken&&r.searchParams.set("checkout_token",t.machine.context.checkoutToken),(t.options.__navigate??(i=>window.location.assign(i)))(r.toString())}function Ie(t){return t==="card"?"card":t}function De(t){let e=t;if(e.status===!1)throw new l({code:"payment_channel_update_failed",message:typeof e.message=="string"&&e.message.trim()?e.message:"We could not set up this payment method."})}function ze(t,e){let n=p(t),r=p(e),a=n.fee??n.transaction_fee,s=r.amount!==void 0&&r.amount_to_be_paid===void 0&&r.amountToBePaid===void 0&&r.transaction_fee===void 0&&r.fee===void 0&&a!==void 0?{...r,amount:n.amount,amount_to_be_paid:r.amount}:r;return{...t,data:{...n,...s}}}function re(t,e){let n=new k({container:t?.container,theme:t?.theme??{},onClose:a=>b(a==="user"),onRetry:()=>{},onChannelSelected:()=>{},onPaymentStarted:()=>{},onRefreshStatus:()=>{}}),r=new C({reference:"",theme:t?.theme??{},status:"failed"});o={options:t??{reference:""},reference:"",modal:n,poller:new P,machine:r,successCalled:!1,errorCalled:!0,closeCalled:!1},n.mount(),n.renderError(e,!1)}function _(t){return t instanceof l?t.toPayload():{code:"checkout_error",message:"We could not continue with this checkout."}}var ie={open:ae,close:b};typeof window<"u"&&(window.PayIslandCheckout=ie);var Ne=ie;return fe(Me);})();
