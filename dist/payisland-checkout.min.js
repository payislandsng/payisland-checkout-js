"use strict";var PayIslandCheckout=(()=>{var E=Object.defineProperty;var de=Object.getOwnPropertyDescriptor;var pe=Object.getOwnPropertyNames;var le=Object.prototype.hasOwnProperty;var ue=(t,e)=>{for(var n in e)E(t,n,{get:e[n],enumerable:!0})},fe=(t,e,n,r)=>{if(e&&typeof e=="object"||typeof e=="function")for(let a of pe(e))!le.call(t,a)&&a!==n&&E(t,a,{get:()=>e[a],enumerable:!(r=de(e,a))||r.enumerable});return t};var he=t=>fe(E({},"__esModule",{value:!0}),t);var De={};ue(De,{close:()=>y,default:()=>Ie,open:()=>ae});var me="https://ags.payislands.com",ge=new Set(["paid","successful","success"]),ye=new Set(["failed","canceled","cancelled","reversed"]),be=new Set(["expired"]),ve=new Set(["pending","unpaid","processing","initiated"]),p=class extends Error{constructor(e){super(e.message),this.name="CheckoutError",this.code=e.code,this.status=e.status,this.requestId=e.requestId}toPayload(){return{code:this.code,message:this.message,status:this.status,requestId:this.requestId}}};function j(t){if(typeof t!="string"||t.trim().length<6)throw new p({code:"invalid_reference",message:"Enter a valid PayIsland transaction reference."});let e=t.trim();if(!/^[a-zA-Z0-9_-]+$/.test(e))throw new p({code:"invalid_reference",message:"The PayIsland transaction reference contains unsupported characters."});return e}function U(t){if(!t)return me;let e=window.location.hostname;if(!["localhost","127.0.0.1","::1"].includes(e))throw new p({code:"api_base_not_allowed",message:"API base overrides are only available for local development and testing."});return t.replace(/\/+$/,"")}function K(t){return t.get("x-request-id")??t.get("x-correlation-id")??t.get("x-trace-id")??void 0}function F(t){let e=String(t??"").trim().toLowerCase();return ge.has(e)?"success":be.has(e)?"expired":ye.has(e)?"failed":ve.has(e)?"pending":"unknown"}function l(t){if(!t)return{};let e="data"in t?t.data:void 0;return("transaction"in t?t.transaction:void 0)??e??t}function H(t){let e=l(t),n=typeof t?.status=="string"?t.status:void 0;return e.status??e.payment_status??n}function J(t){let e=l(t),n=t?.channels??t?.payment_channels??t?.available_channels??e.channels;if(Array.isArray(n)&&n.length>0){let a=n.filter(o=>typeof o=="string").map(v);return[...new Set(a)]}let r=[];return L(t)&&r.push("card"),m(t)&&r.push("bank-transfer"),r.length>0?r:["card","bank-transfer"]}function v(t){let e=String(t).trim().toLowerCase();return e==="redirect"||e==="card-payment"?"card":e==="bank_transfer"?"bank-transfer":t}function G(t){let e=l(t),n=t?.business??e.business??{};return t?.merchant??e.merchant??{business_name:t?.business_name??e.business_name??t?.merchant_name??e.merchant_name??n.business_name??n.merchant_name??n.name,name:t?.merchant_name??e.merchant_name??n.name??n.business_name,logo:t?.business_logo??t?.merchant_logo??e.business_logo??e.merchant_logo??n.business_logo??n.merchant_logo??n.logo_url??n.logo}}function W(t){let e=l(t);return t?.customer??e.customer??{}}function m(t){if(!t)return;let e=B(t);if(e)return e;let n=l(t);if(n!==t)return B(n)}function L(t){if(!t)return;let e=t.authorization_url??t.authorizationUrl;if(e)return e;let n=l(t);if(n!==t)return n.authorization_url??n.authorizationUrl}function x(t,e=5e3){let n=t?.retry_after_ms,r=t?.poll_interval_ms,a=typeof n=="number"?n:r;return!a||a<1e3?e:Math.min(a,6e4)}function w(t,e="NGN"){if(t==null||t==="")return"--";let n=Number(t);return Number.isFinite(n)?new Intl.NumberFormat("en-NG",{style:"currency",currency:e,minimumFractionDigits:2}).format(n):String(t)}function V(t){if(!t)return"";let[e,n]=t.split("@");return!e||!n?t:`${e.slice(0,2)}${"*".repeat(Math.max(e.length-2,2))}@${n}`}function q(t){return(t.name??[t.first_name,t.last_name].filter(Boolean).join(" ")).trim()}function X(t,e){return t.business_name??t.merchant_name??t.name??e??"PayIsland merchant"}function S(t){if(t)try{let e=new URL(t);if(e.protocol==="https:"||e.protocol==="http:"&&xe(e.hostname))return e.toString()}catch{return}}function xe(t){return t==="localhost"||t==="127.0.0.1"||t==="::1"}function B(t){let e=t,r=[t.bank_transfer,t.bankTransfer,e.bank_account,e.bankAccount,e.virtual_account,e.virtualAccount,e.transfer_account,e.transferAccount,e.account_details,e.accountDetails].find(A);return r||(A(t)?t:void 0)}function A(t){if(!t||typeof t!="object")return!1;let e=t;return!!(e.account_number??e.accountNumber??e.account_no??e.accountNo??(e.account&&typeof e.account=="object"&&(e.account.number??e.account.account_number??e.account.accountNumber)))}function Y(t,e){return t===404?new p({code:"reference_not_found",message:"This checkout link is invalid or has expired.",status:t,requestId:e}):t>=400&&t<500?new p({code:"validation_error",message:"We could not continue with this checkout. Please confirm the payment details.",status:t,requestId:e}):new p({code:"network_error",message:"We could not reach PayIsland. Please check your connection and try again.",status:t,requestId:e})}var T=class{constructor(e){this.baseUrl=e}setCheckoutToken(e){this.checkoutToken=e}async bootstrap(e){return this.request(`/api/v1/transactions/gateway/pay/keys/${e}`,{method:"GET"})}async submitPayment(e,n){return this.request(`/api/v1/transactions/gateway/pay/${e}`,{method:"POST",body:JSON.stringify(n)})}async verifyOtp(e,n){return this.request(`/api/v1/transactions/gateway/otp/verify/${e}`,{method:"POST",body:JSON.stringify(n)})}async verify(e){return this.request(`/api/v1/transactions/gateway/transaction/verify/${e}`,{method:"POST",body:JSON.stringify({})})}async verifyBankTransfer(e){return this.request(`/api/v1/transactions/gateway/transaction/verify-bank-transfer/${e}`,{method:"GET"})}async updatePaymentChannel(e,n){return this.request(`/api/v1/transactions/customer/update-payment-channel/${e}`,{method:"POST",body:JSON.stringify({channel:n})})}async request(e,n){let r;try{r=await fetch(`${this.baseUrl}${e}`,{...n,headers:{Accept:"application/json",...n.method!=="GET"?{"Content-Type":"application/json"}:{},...this.checkoutToken?{"x-checkout-token":this.checkoutToken}:{},...n.headers},credentials:"omit"})}catch{throw new p({code:"network_error",message:"We could not reach PayIsland. Please check your connection and try again."})}let a=K(r.headers);if(!r.ok)throw Y(r.status,a);return{data:await we(r)??{},checkoutToken:r.headers.get("x-checkout-token")??void 0,requestId:a}}};async function we(t){if((t.headers.get("content-type")??"").includes("application/json"))return await t.json()}function I(t){let e=t?.account,n=e&&typeof e=="object"?e:void 0,r=t?.bank&&typeof t.bank=="object"?t.bank:void 0,a=t?.account_number??t?.accountNumber??t?.account_no??t?.accountNo??g(n?.number)??g(n?.account_number)??g(n?.accountNumber);return a?{accountNumber:a,accountName:t?.account_name??t?.accountName??t?.name??g(n?.name)??g(n?.account_name)??g(n?.accountName)??"PayIsland checkout",bankName:t?.bank_name??t?.bankName??(typeof t?.bank=="string"?t.bank:void 0)??r?.name??r?.bank_name??r?.bankName??"Bank transfer",expiresAt:t?.expires_at??t?.expiresAt??t?.expiry_date??t?.expiryDate}:null}function g(t){return typeof t=="string"&&t.trim()?t:void 0}function Z(t){let e=S(t);return e?(window.open(e,"_blank","noopener,noreferrer"),!0):!1}var $="data:image/png;base64,";var D="data:image/png;base64,";var Q=`
:host {
  all: initial;
  --pi-primary: #074761;
  --pi-primary-strong: #05364a;
  --pi-primary-soft: #e7f2f6;
  --pi-accent: #8aa0ad;
  --pi-text: #111827;
  --pi-muted: #607080;
  --pi-border: #d8e1e8;
  --pi-border-strong: #b9c7d1;
  --pi-surface: #ffffff;
  --pi-soft: #f6f9fb;
  --pi-raised: #fbfdfe;
  --pi-danger: #b42318;
  --pi-danger-soft: #fff1f0;
  --pi-success: #067647;
  --pi-success-soft: #e8f8ef;
  --pi-warning: #9a6700;
  --pi-warning-soft: #fff7df;
  --pi-focus: rgba(7, 71, 97, 0.28);
  font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
}
* {
  box-sizing: border-box;
}
button,
input {
  font: inherit;
}
button {
  -webkit-tap-highlight-color: transparent;
}
button:focus-visible {
  outline: 3px solid var(--pi-focus);
  outline-offset: 2px;
}
.pi-overlay {
  position: fixed;
  inset: 0;
  z-index: 2147483647;
  display: grid;
  place-items: center;
  padding: 20px;
  background: rgba(10, 20, 30, 0.58);
  backdrop-filter: blur(4px);
}
.pi-inline {
  width: 100%;
}
.pi-modal {
  width: min(100%, 480px);
  max-height: min(780px, calc(100vh - 40px));
  overflow: hidden;
  border: 1px solid rgba(216, 225, 232, 0.92);
  border-radius: 14px;
  background: var(--pi-surface);
  color: var(--pi-text);
  box-shadow: 0 28px 90px rgba(8, 18, 28, 0.28);
}
.pi-inline .pi-modal {
  max-height: none;
  box-shadow: 0 10px 32px rgba(8, 18, 28, 0.08);
}
.pi-header,
.pi-body,
.pi-footer {
  padding: 16px 18px;
}
.pi-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 14px;
  border-bottom: 1px solid var(--pi-border);
  background: linear-gradient(180deg, #ffffff 0%, #f8fbfd 100%);
}
.pi-header-main,
.pi-brand {
  display: flex;
  align-items: center;
  min-width: 0;
  gap: 12px;
}
.pi-logo-frame {
  display: grid;
  width: 42px;
  height: 42px;
  flex: 0 0 auto;
  place-items: center;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: #fff;
  box-shadow: 0 8px 18px rgba(7, 71, 97, 0.08);
}
.pi-logo,
.pi-logo-fallback {
  width: 28px;
  height: 28px;
  flex: 0 0 auto;
  object-fit: contain;
}
.pi-logo-fallback {
  display: grid;
  place-items: center;
  border-radius: 8px;
  background: var(--pi-primary);
  color: #fff;
  font-weight: 800;
}
.pi-heading {
  min-width: 0;
}
.pi-eyebrow,
.pi-label {
  display: block;
  margin: 0 0 3px;
  color: var(--pi-muted);
  font-size: 11px;
  font-weight: 800;
  letter-spacing: 0.04em;
  line-height: 1.2;
  text-transform: uppercase;
}
.pi-title {
  margin: 0;
  overflow-wrap: anywhere;
  color: var(--pi-text);
  font-size: 16px;
  font-weight: 800;
  line-height: 1.25;
}
.pi-subtitle {
  margin: 4px 0 0;
  color: var(--pi-muted);
  font-size: 13px;
  line-height: 1.35;
}
.pi-close {
  display: grid;
  width: 38px;
  height: 38px;
  flex: 0 0 auto;
  place-items: center;
  border: 1px solid transparent;
  border-radius: 999px;
  background: transparent;
  color: var(--pi-muted);
  cursor: pointer;
  font-size: 24px;
  line-height: 1;
}
.pi-close:hover,
.pi-close:focus-visible {
  border-color: var(--pi-border);
  background: #fff;
  color: var(--pi-text);
}
.pi-body {
  max-height: calc(100vh - 178px);
  overflow-y: auto;
  background: var(--pi-surface);
}
.pi-inline .pi-body {
  max-height: none;
}
.pi-stack {
  display: grid;
  gap: 16px;
}
.pi-summary {
  display: grid;
  overflow: hidden;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: var(--pi-raised);
}
.pi-summary-hero {
  display: flex;
  align-items: flex-end;
  justify-content: space-between;
  gap: 16px;
  padding: 14px;
  border-bottom: 1px solid var(--pi-border);
  background: linear-gradient(135deg, var(--pi-primary-soft), #ffffff 70%);
}
.pi-summary-hero strong {
  display: block;
  color: var(--pi-primary-strong);
  font-size: 26px;
  font-weight: 850;
  line-height: 1.05;
}
.pi-summary-hero p {
  max-width: 42%;
  margin: 0;
  color: var(--pi-muted);
  font-size: 13px;
  font-weight: 700;
  line-height: 1.35;
  text-align: right;
}
.pi-summary-grid {
  display: grid;
  gap: 0;
  padding: 4px 14px;
}
.pi-row {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 16px;
  min-height: 34px;
  border-bottom: 1px solid rgba(216, 225, 232, 0.72);
  color: var(--pi-muted);
  font-size: 14px;
}
.pi-row:last-child {
  border-bottom: 0;
}
.pi-row strong {
  min-width: 0;
  color: var(--pi-text);
  font-weight: 750;
  overflow-wrap: anywhere;
  text-align: right;
}
.pi-reference strong {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 700;
}
.pi-tabs {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(124px, 1fr));
  gap: 6px;
  padding: 5px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: var(--pi-soft);
}
.pi-tab {
  min-height: 40px;
  border: 1px solid transparent;
  border-radius: 9px;
  background: transparent;
  color: var(--pi-muted);
  cursor: pointer;
  font-size: 13px;
  font-weight: 800;
}
.pi-tab:hover:not([disabled]) {
  color: var(--pi-text);
  background: #fff;
}
.pi-tab[aria-selected="true"] {
  border-color: rgba(7, 71, 97, 0.18);
  background: var(--pi-primary);
  color: #fff;
  box-shadow: 0 8px 18px rgba(7, 71, 97, 0.18);
}
.pi-tab[disabled] {
  cursor: not-allowed;
  opacity: 0.48;
}
.pi-panel {
  display: grid;
  gap: 12px;
}
.pi-panel-title,
.pi-state-title {
  margin: 0;
  color: var(--pi-text);
  font-size: 16px;
  font-weight: 850;
  line-height: 1.25;
}
.pi-bank-box {
  display: grid;
  gap: 10px;
  padding: 14px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: #fff;
  box-shadow: 0 10px 24px rgba(8, 18, 28, 0.05);
}
.pi-bank-intro {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
.pi-bank-intro strong {
  color: var(--pi-primary-strong);
  font-size: 14px;
  font-weight: 850;
  text-align: right;
}
.pi-account {
  display: grid;
  gap: 8px;
  padding: 12px;
  border: 1px solid rgba(7, 71, 97, 0.16);
  border-radius: 12px;
  background: linear-gradient(180deg, var(--pi-primary-soft), #fff);
}
.pi-account > span {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 800;
  letter-spacing: 0.04em;
  text-transform: uppercase;
}
.pi-account-number {
  color: var(--pi-primary-strong);
  font-size: 28px;
  font-weight: 900;
  letter-spacing: 0;
  line-height: 1.1;
  overflow-wrap: anywhere;
}
.pi-bank-grid {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 10px;
}
.pi-detail {
  display: grid;
  gap: 5px;
  min-width: 0;
  padding: 12px;
  border: 1px solid var(--pi-border);
  border-radius: 10px;
  background: var(--pi-soft);
}
.pi-detail span {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 750;
}
.pi-detail strong {
  min-width: 0;
  color: var(--pi-text);
  font-size: 14px;
  font-weight: 800;
  overflow-wrap: anywhere;
}
.pi-copy,
.pi-primary,
.pi-secondary {
  min-height: 42px;
  border-radius: 10px;
  cursor: pointer;
  font-weight: 800;
}
.pi-copy {
  justify-self: start;
  border: 1px solid var(--pi-primary);
  background: #fff;
  color: var(--pi-primary);
  padding: 0 14px;
}
.pi-copy:hover,
.pi-secondary:hover {
  border-color: var(--pi-border-strong);
  background: var(--pi-soft);
}
.pi-primary {
  width: 100%;
  border: 0;
  background: var(--pi-primary);
  color: #fff;
  box-shadow: 0 10px 18px rgba(7, 71, 97, 0.18);
}
.pi-primary:hover,
.pi-primary:focus-visible {
  background: var(--pi-primary-strong);
}
.pi-primary:disabled,
.pi-secondary:disabled {
  cursor: not-allowed;
  opacity: 0.62;
  box-shadow: none;
}
.pi-secondary {
  border: 1px solid var(--pi-border);
  background: #fff;
  color: var(--pi-text);
  padding: 0 14px;
}
.pi-secondary:disabled {
  cursor: wait;
}
.pi-status {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
  padding: 11px 12px;
  border: 1px solid rgba(154, 103, 0, 0.2);
  border-radius: 10px;
  background: var(--pi-warning-soft);
  color: #6f4a00;
  font-size: 13px;
}
.pi-status strong {
  color: #5f3f00;
  font-weight: 850;
  text-align: right;
  text-transform: capitalize;
}
.pi-bank-actions {
  display: grid;
}
.pi-refresh {
  width: 100%;
}
.pi-redirect-card {
  display: grid;
  gap: 14px;
  padding: 16px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: linear-gradient(180deg, #ffffff, var(--pi-soft));
}
.pi-redirect-icon {
  display: grid;
  width: 42px;
  height: 42px;
  place-items: center;
  border-radius: 999px;
  background: var(--pi-primary-soft);
  color: var(--pi-primary);
  font-size: 20px;
  font-weight: 900;
}
.pi-state {
  display: grid;
  justify-items: center;
  gap: 10px;
  padding: 34px 14px;
  text-align: center;
}
.pi-state-logo {
  width: 36px;
  height: 36px;
  object-fit: contain;
}
.pi-spinner {
  width: 34px;
  height: 34px;
  border: 3px solid var(--pi-border);
  border-top-color: var(--pi-primary);
  border-radius: 50%;
  animation: pi-spin 0.8s linear infinite;
}
.pi-badge {
  display: inline-grid;
  place-items: center;
  min-width: 40px;
  height: 40px;
  border-radius: 999px;
  font-weight: 900;
}
.pi-badge-success {
  background: var(--pi-success-soft);
  color: var(--pi-success);
}
.pi-badge-error {
  background: var(--pi-danger-soft);
  color: var(--pi-danger);
}
.pi-badge-warning {
  background: var(--pi-warning-soft);
  color: var(--pi-warning);
}
.pi-message {
  margin: 0;
  color: var(--pi-muted);
  font-size: 14px;
  line-height: 1.5;
}
.pi-error {
  padding: 12px;
  border: 1px solid #fecdca;
  border-radius: 10px;
  background: #fffbfa;
  color: var(--pi-danger);
  font-size: 14px;
}
.pi-unavailable {
  padding: 14px;
  border: 1px dashed var(--pi-border-strong);
  border-radius: 10px;
  background: var(--pi-soft);
  color: var(--pi-muted);
  font-size: 14px;
}
.pi-footer {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
  border-top: 1px solid var(--pi-border);
  background: var(--pi-raised);
}
.pi-secured {
  display: inline-flex;
  align-items: center;
  min-width: 0;
  gap: 7px;
  padding: 7px 10px;
  border-radius: 999px;
  background: var(--pi-primary-strong);
  color: rgba(255, 255, 255, 0.78);
  font-size: 12px;
  line-height: 1;
}
.pi-secured strong {
  color: #fff;
  font-weight: 850;
}
.pi-secured-logo {
  width: 15px;
  height: 20px;
  object-fit: contain;
}
.pi-sr-only {
  position: absolute;
  width: 1px;
  height: 1px;
  padding: 0;
  overflow: hidden;
  clip: rect(0, 0, 0, 0);
  white-space: nowrap;
  border: 0;
}
@keyframes pi-spin {
  to {
    transform: rotate(360deg);
  }
}
@media (max-width: 520px) {
  .pi-overlay {
    align-items: end;
    padding: 10px;
  }
  .pi-modal {
    width: 100%;
    max-height: calc(100vh - 20px);
    border-radius: 14px 14px 10px 10px;
  }
  .pi-header,
  .pi-body,
  .pi-footer {
    padding: 15px;
  }
  .pi-header {
    gap: 10px;
  }
  .pi-logo-frame {
    width: 38px;
    height: 38px;
    border-radius: 10px;
  }
  .pi-title {
    font-size: 15px;
  }
  .pi-summary-hero {
    align-items: flex-start;
    flex-direction: column;
    gap: 8px;
  }
  .pi-summary-hero p {
    max-width: none;
    text-align: left;
  }
  .pi-summary-hero strong {
    font-size: 25px;
  }
  .pi-bank-intro,
  .pi-status {
    align-items: flex-start;
    flex-direction: column;
    gap: 6px;
  }
  .pi-bank-intro strong,
  .pi-status strong {
    text-align: left;
  }
  .pi-bank-grid {
    grid-template-columns: 1fr;
  }
  .pi-account-number {
    font-size: 26px;
  }
  .pi-tabs {
    grid-template-columns: 1fr;
  }
  .pi-footer {
    align-items: stretch;
    flex-direction: column;
  }
  .pi-secured {
    justify-content: center;
  }
}
`;var ee=["a[href]","button:not([disabled])","input:not([disabled])","select:not([disabled])","textarea:not([disabled])",'[tabindex]:not([tabindex="-1"])'].join(","),k=class{constructor(e){this.inline=!1;this.refreshingStatus=!1;this.options=e,this.host=document.createElement("div"),this.host.setAttribute("data-payisland-checkout",""),this.root=this.host.attachShadow({mode:"open"})}mount(){this.previousActiveElement=document.activeElement;let e=this.resolveContainer();this.inline=!!e,(e??document.body).appendChild(this.host),this.renderShell(),this.focusFirst()}destroy(){this.stopCountdown(),this.host.remove(),this.previousActiveElement instanceof HTMLElement&&this.previousActiveElement.focus({preventScroll:!0})}renderLoading(){this.setBody(`
      <div class="pi-state pi-state-loading" role="status" aria-live="polite">
        <img class="pi-state-logo" src="${h($)}" alt="" />
        <div class="pi-spinner" aria-hidden="true"></div>
        <h3 class="pi-state-title">Preparing checkout</h3>
        <p class="pi-message">Preparing secure checkout...</p>
      </div>
    `)}getSelectedChannel(){return this.selectedChannel}renderError(e,n=!0){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        <div class="pi-badge pi-badge-error" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Checkout unavailable</h3>
        <p class="pi-message">${c(e.message)}</p>
        ${n?'<button class="pi-secondary" type="button" data-action="retry">Retry</button>':""}
      </div>
    `)}renderCheckout(e,n,r){let a=this.filterChannels(J(e),n,e);return a.length===0?(this.selectedChannel=void 0,this.renderError({code:"no_available_channels",message:"No available payment channels for this checkout."},!1),!1):(this.selectedChannel=this.selectedChannel&&a.includes(this.selectedChannel)?this.selectedChannel:a.find(o=>this.isSupported(o,e))??a[0],this.setBody(`
      <div class="pi-stack">
        ${this.renderSummary(e)}
        ${this.renderTabs(a,e)}
        <div class="pi-panel">${this.renderChannel(e,this.selectedChannel,r)}</div>
      </div>
    `),this.startCountdown(e),!0)}renderPending(e){this.setBody(`
      <div class="pi-state pi-state-pending" role="status" aria-live="polite">
        <div class="pi-spinner" aria-hidden="true"></div>
        <h3 class="pi-state-title">Confirming payment</h3>
        <p class="pi-message">Payment is pending. We are checking for confirmation.</p>
        ${e?.status?`<p class="pi-subtitle">Status: ${c(e.status)}</p>`:""}
      </div>
    `)}renderSuccess(){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-success" role="status" aria-live="polite">
        <div class="pi-badge pi-badge-success" aria-hidden="true">\u2713</div>
        <h3 class="pi-state-title">Payment successful</h3>
        <p class="pi-message">Payment successful.</p>
      </div>
    `)}renderFailure(e){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        <div class="pi-badge pi-badge-error" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Payment failed</h3>
        <p class="pi-message">${c(e)}</p>
      </div>
    `)}renderExpired(){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-expired" role="alert">
        <div class="pi-badge pi-badge-warning" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Checkout expired</h3>
        <p class="pi-message">This checkout has expired. Please start a new payment.</p>
      </div>
    `)}setStatusRefreshing(e){this.refreshingStatus=e;let n=this.root.querySelector('[data-action="refresh-status"]');n&&(n.disabled=e,n.textContent=e?"Checking...":"Refresh status")}setChannelLoading(e,n){this.loadingChannel=n?e:void 0}renderShell(){let e=this.options.theme.merchantName??"PayIsland Checkout",n=this.options.theme.logoUrl??$,r=this.inline?"pi-inline":"pi-overlay";this.root.innerHTML=`
      <style>${Q}</style>
      <div class="${r}" data-shell>
        <section class="pi-modal" role="dialog" aria-modal="${this.inline?"false":"true"}" aria-labelledby="pi-title">
          <header class="pi-header">
            <div class="pi-header-main">
              <div class="pi-logo-frame">
                <img class="pi-logo" src="${h(n)}" alt="" />
              </div>
              <div class="pi-heading">
                <p class="pi-eyebrow">Secure checkout</p>
                <h2 class="pi-title" id="pi-title">${c(e)}</h2>
                <p class="pi-subtitle">Protected by PayIsland</p>
              </div>
            </div>
            <button class="pi-close" type="button" aria-label="Close checkout" data-action="close">
              <span aria-hidden="true">\xD7</span>
            </button>
          </header>
          <main class="pi-body" data-body></main>
          <footer class="pi-footer">
            <div class="pi-secured">
              <span>Secured by</span>
              <img class="pi-secured-logo" src="${h(D)}" alt="PayIsland" />
              <strong>PayIsland</strong>
            </div>
            <button class="pi-secondary" type="button" data-action="close">Cancel</button>
          </footer>
        </section>
      </div>
    `,this.root.querySelector("[data-shell]")?.addEventListener("click",a=>{!this.inline&&a.target===a.currentTarget&&this.options.onClose("user")}),this.root.addEventListener("click",a=>this.handleClick(a)),this.root.addEventListener("keydown",a=>this.handleKeydown(a)),this.options.theme.primaryColor&&this.host.style.setProperty("--pi-primary",this.options.theme.primaryColor)}setBody(e){let n=this.root.querySelector("[data-body]");n&&(n.innerHTML=e)}renderSummary(e){let n=l(e),r=G(e),a=W(e),o=n.currency??e.currency??"NGN",i=n.total_amount??n.totalAmount??n.amount_to_be_paid??n.amountToBePaid??e.total_amount??e.totalAmount??e.amount_to_be_paid??e.amountToBePaid,d=n.amount??e.amount,u=n.fee??n.transaction_fee??e.fee??e.transaction_fee,f=q(a),b=V(a.email),se=X(r,this.options.theme.merchantName),M=n.reference??e.reference??"",ce=w(i??te(d,u)??d,o);return`
      <section class="pi-summary" aria-label="Transaction summary">
        <div class="pi-summary-hero">
          <div>
            <span class="pi-label">Total due</span>
            <strong>${c(ce)}</strong>
          </div>
          <p>${c(se)}</p>
        </div>
        <div class="pi-summary-grid">
          <div class="pi-row"><span>Amount</span><strong>${c(w(d,o))}</strong></div>
          <div class="pi-row"><span>Fee</span><strong>${c(w(u,o))}</strong></div>
          ${f?`<div class="pi-row"><span>Customer</span><strong>${c(f)}</strong></div>`:""}
          ${b?`<div class="pi-row"><span>Email</span><strong>${c(b)}</strong></div>`:""}
          ${M?`<div class="pi-row pi-reference"><span>Reference</span><strong>${c(M)}</strong></div>`:""}
        </div>
      </section>
    `}renderTabs(e,n){return`
      <div class="pi-tabs" role="tablist" aria-label="Payment channels">
        ${e.map(r=>{let a=this.isSupported(r,n);return`
              <button
                class="pi-tab"
                type="button"
                role="tab"
                aria-selected="${r===this.selectedChannel?"true":"false"}"
                data-channel="${h(r)}"
                ${a?"":"disabled"}
              >
                ${c(z(r))}
              </button>
            `}).join("")}
      </div>
    `}renderChannel(e,n,r){if(n&&this.loadingChannel===n)return`
        <div class="pi-state pi-state-loading" role="status" aria-live="polite">
          <div class="pi-spinner" aria-hidden="true"></div>
          <h3 class="pi-state-title">Preparing ${c(z(n))}</h3>
          <p class="pi-message">Setting up this payment method...</p>
        </div>
      `;if(n==="bank-transfer"){let a=I(m(e)),o=l(e),i=o.currency??e.currency??"NGN",d=o.amount??e.amount,u=o.fee??o.transaction_fee??e.fee??e.transaction_fee,f=o.total_amount??o.totalAmount??o.amount_to_be_paid??o.amountToBePaid??e.total_amount??e.totalAmount??e.amount_to_be_paid??e.amountToBePaid??te(d,u)??d;return a?`
        <section class="pi-bank-box" aria-label="Bank transfer details">
          <div class="pi-bank-intro">
            <span class="pi-label">Bank transfer</span>
            <strong>Transfer exactly ${c(w(f,i))}</strong>
          </div>
          <div class="pi-account">
            <span>Account number</span>
            <strong class="pi-account-number">${c(a.accountNumber)}</strong>
            <button class="pi-copy" type="button" data-copy="${h(a.accountNumber)}">Copy number</button>
          </div>
          <div class="pi-bank-grid">
            <div class="pi-detail"><span>Bank</span><strong>${c(a.bankName)}</strong></div>
            <div class="pi-detail"><span>Account name</span><strong>${c(a.accountName)}</strong></div>
          </div>
          ${a.expiresAt?`<p class="pi-subtitle" data-countdown="${h(a.expiresAt)}"></p>`:""}
          <div class="pi-status" role="status" aria-live="polite">
            <span>Current status</span>
            <strong>${c(Ce(r))}</strong>
          </div>
          <div class="pi-bank-actions">
            <button class="pi-secondary pi-refresh" type="button" data-action="refresh-status" ${this.refreshingStatus?"disabled":""}>
              ${this.refreshingStatus?"Checking...":"Refresh status"}
            </button>
          </div>
          <p class="pi-message">Keep this checkout open while we confirm your payment.</p>
        </section>
      `:`
          <section class="pi-redirect-card">
            <div class="pi-redirect-icon" aria-hidden="true">\u2197</div>
            <div>
              <h3 class="pi-panel-title">Generate transfer details</h3>
              <p class="pi-message">Generate a dedicated account number and transfer the exact amount.</p>
            </div>
            <button class="pi-primary" type="button" data-action="initialize-channel" data-channel-target="bank-transfer">
              Continue to transfer
            </button>
          </section>
        `}if(n==="card"){let a=S(L(e));return`
        <section class="pi-redirect-card">
          <div class="pi-redirect-icon" aria-hidden="true">\u2197</div>
          <div>
            <h3 class="pi-panel-title">Card payment</h3>
            <p class="pi-message">Continue to the secure PayIsland card checkout to enter card details and authorize payment.</p>
          </div>
          <button
            class="pi-primary"
            type="button"
            ${a?`data-card-redirect="${h(a)}"`:'data-action="initialize-channel" data-channel-target="card"'}
          >
            Continue to card payment
          </button>
        </section>
      `}return`<div class="pi-unavailable">${c(z(n??"This channel"))} is not available in this checkout yet.</div>`}startCountdown(e){this.stopCountdown();let n=I(m(e));if(!n?.expiresAt)return;let r=new Date(n.expiresAt).getTime(),a=this.root.querySelector("[data-countdown]");if(!Number.isFinite(r)||!a)return;let o=()=>{let i=Math.max(0,r-Date.now()),d=Math.floor(i/6e4),u=Math.floor(i%6e4/1e3);a.textContent=i>0?`Transfer account expires in ${d}:${u.toString().padStart(2,"0")}`:"Transfer account has expired."};o(),this.countdownTimer=window.setInterval(o,1e3)}stopCountdown(){this.countdownTimer&&(window.clearInterval(this.countdownTimer),this.countdownTimer=void 0)}filterChannels(e,n,r){let a=[...new Set(e.map(v))];if(n?.length){let o=n.map(v);return a.filter(i=>o.includes(i)&&(r?this.isSupported(i,r):!0))}return a.length>0?a:["card","bank-transfer"]}isSupported(e,n){return e==="bank-transfer"||e==="card"||e==="mono"}handleClick(e){let n=e.target;if(!(n instanceof HTMLElement))return;if(n.closest('[data-action="close"]')){this.options.onClose("user");return}if(n.closest('[data-action="retry"]')){this.options.onRetry();return}if(n.closest('[data-action="refresh-status"]')){this.selectedChannel&&this.options.onPaymentStarted(this.selectedChannel),this.options.onRefreshStatus();return}let i=n.closest('[data-action="initialize-channel"]')?.dataset.channelTarget;if(i){this.selectedChannel=i,this.options.onChannelSelected(i);return}let d=n.closest("[data-channel]")?.dataset.channel;if(d){let b=v(d);this.selectedChannel=b,this.options.onChannelSelected(b);return}let u=n.closest("[data-copy]")?.dataset.copy;if(u){navigator.clipboard?.writeText(u),n.textContent="Copied";return}let f=n.closest("[data-card-redirect]")?.dataset.cardRedirect;f&&(Z(f),this.options.onPaymentStarted(this.selectedChannel??"card"))}handleKeydown(e){if(e.key==="Escape"){this.options.onClose("user");return}if(e.key!=="Tab"||this.inline)return;let n=Array.from(this.root.querySelectorAll(ee));if(n.length===0)return;let r=n[0],a=n[n.length-1],o=this.root.activeElement;e.shiftKey&&o===r?(e.preventDefault(),a.focus()):!e.shiftKey&&o===a&&(e.preventDefault(),r.focus())}focusFirst(){window.setTimeout(()=>{this.root.querySelector(ee)?.focus()},0)}resolveContainer(){let e=this.options.container;if(e)return e instanceof HTMLElement?e:document.querySelector(e)??void 0}};function z(t){return{"bank-transfer":"Bank transfer",redirect:"Card",card:"Card",mono:"Mono"}[t]??String(t)}function Ce(t){if(!t)return"Pending confirmation";let e=String(t).trim();return e?e.replace(/[-_]+/g," "):"Pending confirmation"}function te(t,e){if(t==null||t===""||e==null||e==="")return;let n=Number(t),r=Number(e);if(!(!Number.isFinite(n)||!Number.isFinite(r)))return Math.round((n+r)*100)/100}function c(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function h(t){return c(t)}var P=class{constructor(){this.stopped=!0}start(e,n,r=0){this.stop(),this.stopped=!1;let a=async()=>{if(!this.stopped)try{let o=await e();if(this.stopped||(n(o),this.stopped))return;this.timer=window.setTimeout(a,x(o))}catch{this.stopped||(this.timer=window.setTimeout(a,5e3))}};this.timer=window.setTimeout(a,r)}stop(){this.stopped=!0,this.timer&&(window.clearTimeout(this.timer),this.timer=void 0)}};var _e={idle:{LOAD:"loading",CLOSE:"closed"},loading:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},ready:{PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},pending:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},success:{CLOSE:"closed"},failed:{CLOSE:"closed"},expired:{CLOSE:"closed"},closed:{}},C=class{constructor(e){this.context=e}send(e){let n=_e[this.context.status][e.type];return n&&(this.context.status=n),this.context.status}isTerminal(){return this.context.status==="success"||this.context.status==="failed"||this.context.status==="expired"||this.context.status==="closed"}};var s;function ae(t){y(!1);let e;try{e=j(t?.reference)}catch(i){let d=R(i);t?.onError?.(d),re(t,d);return}let n;try{n=new T(U(t.__apiBaseUrl))}catch(i){let d=R(i);t.onError?.(d),re(t,d);return}let r=new C({reference:e,theme:t.theme??{},status:"idle"}),a=new P,o=new k({container:t.container,theme:t.theme??{},onClose:i=>y(i==="user"),onRetry:()=>{ne()},onChannelSelected:i=>{s?.bootstrap&&(s.machine.context.selectedChannel=i,s.modal.renderCheckout(s.bootstrap,s.options.channels,s.machine.context.status),Se(i))},onPaymentStarted:()=>Te(),onRefreshStatus:()=>{$e()}});s={options:t,reference:e,api:n,modal:o,poller:a,machine:r,successCalled:!1,errorCalled:!1,closeCalled:!1},o.mount(),ne()}function y(t=!0){if(!s)return;let e=s;s=void 0,e.poller.stop(),e.machine.send({type:"CLOSE"}),e.modal.destroy(),e.api?.setCheckoutToken(void 0),e.bootstrap=void 0,t&&!e.closeCalled&&(e.closeCalled=!0,e.options.onClose?.())}async function ne(){let t=s;if(!(!t||!t.api)){t.poller.stop(),t.machine.send({type:"LOAD"}),t.modal.renderLoading();try{let e=await t.api.bootstrap(t.reference);if(s!==t)return;if(t.bootstrap=e.data,t.machine.context.bootstrap=e.data,t.machine.context.checkoutToken=e.checkoutToken,t.api.setCheckoutToken(e.checkoutToken),!t.modal.renderCheckout(e.data,t.options.channels)){t.machine.send({type:"FAIL"}),_({code:"no_available_channels",message:"No available payment channels for this checkout."});return}O(e.data,!1)}catch(e){if(s!==t)return;let n=R(e);t.machine.context.error=n,t.machine.send({type:"FAIL"}),t.modal.renderError(n),_(n)}}}async function Se(t){let e=s;if(!(!e||!e.api||!e.bootstrap)){e.modal.setChannelLoading(t,!0),e.modal.renderCheckout(e.bootstrap,e.options.channels,e.machine.context.status);try{let n=await e.api.updatePaymentChannel(e.reference,Ee(t));if(s!==e)return;He(n.data),e.bootstrap=Le(e.bootstrap,n.data),e.machine.context.bootstrap=e.bootstrap}catch(n){let r=R(n);_(r)}finally{s===e&&e.bootstrap&&(e.modal.setChannelLoading(t,!1),e.modal.renderCheckout(e.bootstrap,e.options.channels,e.machine.context.status))}}}function Te(){let t=s;!t||!t.bootstrap||t.machine.isTerminal()||N(x(t.bootstrap))}function N(t){let e=s;!e||!e.api||e.poller.start(()=>oe(e),n=>O(n,!0),t)}async function $e(){let t=s;if(!(!t||!t.api)){t.poller.stop(),t.modal.setStatusRefreshing(!0);try{let e=await oe(t);if(s!==t)return;O(e,!0),t.machine.isTerminal()||N(x(e))}catch{s===t&&!t.machine.isTerminal()&&N(5e3)}finally{s===t&&t.modal.setStatusRefreshing(!1)}}}async function oe(t){if(!t.api)return{};if(t.modal.getSelectedChannel()==="bank-transfer"&&m(t.bootstrap))try{return(await t.api.verifyBankTransfer(t.reference)).data}catch{return(await t.api.verify(t.reference)).data}return(await t.api.verify(t.reference)).data}function O(t,e){let n=s;if(!n)return;let r=F(H(t)),a=l(t);if(r==="success"){n.poller.stop(),n.machine.send({type:"SUCCESS"}),n.successCalled||(n.successCalled=!0,n.options.onSuccess?.(a)),y(!1),Re(n,a);return}if(r==="expired"){n.poller.stop(),n.machine.send({type:"EXPIRE"}),n.modal.renderExpired(),_({code:"payment_expired",message:"This checkout has expired. Please start a new payment."});return}if(r==="failed"){n.poller.stop(),n.machine.send({type:"FAIL"}),n.modal.renderFailure("This payment could not be completed."),_({code:"payment_failed",message:"This payment could not be completed."});return}e?(n.machine.send({type:"PENDING"}),n.bootstrap?n.modal.renderCheckout(n.bootstrap,n.options.channels,H(t)??r):n.modal.renderPending(t),n.options.onPending?.(t)):n.machine.send({type:"READY"})}function _(t){let e=s;!e||e.errorCalled||(e.errorCalled=!0,e.options.onError?.(t))}function Re(t,e){if(typeof window>"u")return;let n=typeof e.reference=="string"&&e.reference.trim()?e.reference:t.reference,r=new URL("/payment-status",t.options.__receiptBaseUrl??"https://checkout.payislands.com");r.searchParams.set("reference",n),r.searchParams.set("status","success"),t.machine.context.checkoutToken&&r.searchParams.set("checkout_token",t.machine.context.checkoutToken),(t.options.__navigate??(o=>window.location.assign(o)))(r.toString())}function Ee(t){return t==="card"?"card":t}function He(t){let e=t;if(e.status===!1)throw new p({code:"payment_channel_update_failed",message:typeof e.message=="string"&&e.message.trim()?e.message:"We could not set up this payment method."})}function Le(t,e){let n=l(t),r=l(e),a=n.fee??n.transaction_fee,i=r.amount!==void 0&&r.amount_to_be_paid===void 0&&r.amountToBePaid===void 0&&r.transaction_fee===void 0&&r.fee===void 0&&a!==void 0?{...r,amount:n.amount,amount_to_be_paid:r.amount}:r;return{...t,data:{...n,...i}}}function re(t,e){let n=new k({container:t?.container,theme:t?.theme??{},onClose:a=>y(a==="user"),onRetry:()=>{},onChannelSelected:()=>{},onPaymentStarted:()=>{},onRefreshStatus:()=>{}}),r=new C({reference:"",theme:t?.theme??{},status:"failed"});s={options:t??{reference:""},reference:"",modal:n,poller:new P,machine:r,successCalled:!1,errorCalled:!0,closeCalled:!1},n.mount(),n.renderError(e,!1)}function R(t){return t instanceof p?t.toPayload():{code:"checkout_error",message:"We could not continue with this checkout."}}var ie={open:ae,close:y};typeof window<"u"&&(window.PayIslandCheckout=ie);var Ie=ie;return he(De);})();
