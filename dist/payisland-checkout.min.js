"use strict";var PayIslandCheckout=(()=>{var $=Object.defineProperty;var oe=Object.getOwnPropertyDescriptor;var ae=Object.getOwnPropertyNames;var se=Object.prototype.hasOwnProperty;var ce=(t,e)=>{for(var r in e)$(t,r,{get:e[r],enumerable:!0})},pe=(t,e,r,i)=>{if(e&&typeof e=="object"||typeof e=="function")for(let n of ae(e))!se.call(t,n)&&n!==r&&$(t,n,{get:()=>e[n],enumerable:!(i=oe(e,n))||i.enumerable});return t};var de=t=>pe($({},"__esModule",{value:!0}),t);var Ce={};ce(Ce,{close:()=>k,default:()=>Pe,open:()=>ee});var le="https://ags.payislands.com",ue=new Set(["paid","successful","success"]),fe=new Set(["failed","canceled","cancelled","reversed"]),he=new Set(["expired"]),me=new Set(["pending","unpaid","processing","initiated"]),d=class extends Error{constructor(e){super(e.message),this.name="CheckoutError",this.code=e.code,this.status=e.status,this.requestId=e.requestId}toPayload(){return{code:this.code,message:this.message,status:this.status,requestId:this.requestId}}};function M(t){if(typeof t!="string"||t.trim().length<6)throw new d({code:"invalid_reference",message:"Enter a valid PayIsland transaction reference."});let e=t.trim();if(!/^[a-zA-Z0-9_-]+$/.test(e))throw new d({code:"invalid_reference",message:"The PayIsland transaction reference contains unsupported characters."});return e}function N(t){if(!t)return le;let e=window.location.hostname;if(!["localhost","127.0.0.1","::1"].includes(e))throw new d({code:"api_base_not_allowed",message:"API base overrides are only available for local development and testing."});return t.replace(/\/+$/,"")}function j(t){return t.get("x-request-id")??t.get("x-correlation-id")??t.get("x-trace-id")??void 0}function R(t){let e=String(t??"").trim().toLowerCase();return ue.has(e)?"success":he.has(e)?"expired":fe.has(e)?"failed":me.has(e)?"pending":"unknown"}function l(t){if(!t)return{};let e="data"in t?t.data:void 0;return("transaction"in t?t.transaction:void 0)??e??t}function P(t){let e=l(t);return t?.status??e.status}function B(t){let e=l(t),r=t?.channels??t?.payment_channels??t?.available_channels??e.channels;if(Array.isArray(r)&&r.length>0)return r.filter(n=>typeof n=="string");let i=[];return u(t)&&i.push("bank-transfer"),g(t)&&i.push("redirect"),i.length>0?i:["bank-transfer","redirect","card"]}function K(t){let e=l(t);return t?.merchant??e.merchant??{}}function A(t){let e=l(t);return t?.customer??e.customer??{}}function u(t){return t?.bank_transfer??t?.bankTransfer}function g(t){return t?.authorization_url??t?.authorizationUrl}function y(t,e=5e3){let r=t?.retry_after_ms,i=t?.poll_interval_ms,n=typeof r=="number"?r:i;return!n||n<1e3?e:Math.min(n,6e4)}function b(t,e="NGN"){if(t==null||t==="")return"--";let r=Number(t);return Number.isFinite(r)?new Intl.NumberFormat("en-NG",{style:"currency",currency:e,minimumFractionDigits:2}).format(r):String(t)}function U(t){if(!t)return"";let[e,r]=t.split("@");return!e||!r?t:`${e.slice(0,2)}${"*".repeat(Math.max(e.length-2,2))}@${r}`}function F(t){return(t.name??[t.first_name,t.last_name].filter(Boolean).join(" ")).trim()}function J(t,e){return t.business_name??t.name??e??"PayIsland merchant"}function m(t){if(t)try{let e=new URL(t);if(e.protocol==="https:"||e.protocol==="http:"&&ge(e.hostname))return e.toString()}catch{return}}function ge(t){return t==="localhost"||t==="127.0.0.1"||t==="::1"}function G(t,e){return t===404?new d({code:"reference_not_found",message:"This checkout link is invalid or has expired.",status:t,requestId:e}):t>=400&&t<500?new d({code:"validation_error",message:"We could not continue with this checkout. Please confirm the payment details.",status:t,requestId:e}):new d({code:"network_error",message:"We could not reach PayIsland. Please check your connection and try again.",status:t,requestId:e})}var C=class{constructor(e){this.baseUrl=e}setCheckoutToken(e){this.checkoutToken=e}async bootstrap(e){return this.request(`/api/v1/transactions/gateway/pay/keys/${e}`,{method:"GET"})}async submitPayment(e,r){return this.request(`/api/v1/transactions/gateway/pay/${e}`,{method:"POST",body:JSON.stringify(r)})}async verifyOtp(e,r){return this.request(`/api/v1/transactions/gateway/otp/verify/${e}`,{method:"POST",body:JSON.stringify(r)})}async verify(e){return this.request(`/api/v1/transactions/gateway/transaction/verify/${e}`,{method:"POST",body:JSON.stringify({})})}async verifyBankTransfer(e){return this.request(`/api/v1/transactions/gateway/transaction/verify-bank-transfer/${e}`,{method:"GET"})}async request(e,r){let i;try{i=await fetch(`${this.baseUrl}${e}`,{...r,headers:{Accept:"application/json",...r.method!=="GET"?{"Content-Type":"application/json"}:{},...this.checkoutToken?{"x-checkout-token":this.checkoutToken}:{},...r.headers},credentials:"omit"})}catch{throw new d({code:"network_error",message:"We could not reach PayIsland. Please check your connection and try again."})}let n=j(i.headers);if(!i.ok)throw G(i.status,n);return{data:await ye(i)??{},checkoutToken:i.headers.get("x-checkout-token")??void 0,requestId:n}}};async function ye(t){if((t.headers.get("content-type")??"").includes("application/json"))return await t.json()}function E(t){let e=t?.account_number??t?.accountNumber;return e?{accountNumber:e,accountName:t?.account_name??t?.accountName??"PayIsland checkout",bankName:t?.bank_name??t?.bankName??"Bank transfer",expiresAt:t?.expires_at??t?.expiresAt}:null}function W(){return"Card payments are not available in this checkout yet."}function V(t){let e=m(t);return e?(window.open(e,"_blank","noopener,noreferrer"),!0):!1}var S="data:image/png;base64,";var H="data:image/png;base64,";var q=`
:host {
  all: initial;
  --pi-primary: #074761;
  --pi-primary-strong: #05364a;
  --pi-primary-soft: #e7f2f6;
  --pi-accent: #8aa0ad;
  --pi-text: #111827;
  --pi-muted: #607080;
  --pi-border: #d8e1e8;
  --pi-border-strong: #b9c7d1;
  --pi-surface: #ffffff;
  --pi-soft: #f6f9fb;
  --pi-raised: #fbfdfe;
  --pi-danger: #b42318;
  --pi-danger-soft: #fff1f0;
  --pi-success: #067647;
  --pi-success-soft: #e8f8ef;
  --pi-warning: #9a6700;
  --pi-warning-soft: #fff7df;
  --pi-focus: rgba(7, 71, 97, 0.28);
  font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
}
* {
  box-sizing: border-box;
}
button,
input {
  font: inherit;
}
button {
  -webkit-tap-highlight-color: transparent;
}
button:focus-visible {
  outline: 3px solid var(--pi-focus);
  outline-offset: 2px;
}
.pi-overlay {
  position: fixed;
  inset: 0;
  z-index: 2147483647;
  display: grid;
  place-items: center;
  padding: 20px;
  background: rgba(10, 20, 30, 0.58);
  backdrop-filter: blur(4px);
}
.pi-inline {
  width: 100%;
}
.pi-modal {
  width: min(100%, 480px);
  max-height: min(780px, calc(100vh - 40px));
  overflow: hidden;
  border: 1px solid rgba(216, 225, 232, 0.92);
  border-radius: 14px;
  background: var(--pi-surface);
  color: var(--pi-text);
  box-shadow: 0 28px 90px rgba(8, 18, 28, 0.28);
}
.pi-inline .pi-modal {
  max-height: none;
  box-shadow: 0 10px 32px rgba(8, 18, 28, 0.08);
}
.pi-header,
.pi-body,
.pi-footer {
  padding: 16px 18px;
}
.pi-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 14px;
  border-bottom: 1px solid var(--pi-border);
  background: linear-gradient(180deg, #ffffff 0%, #f8fbfd 100%);
}
.pi-header-main,
.pi-brand {
  display: flex;
  align-items: center;
  min-width: 0;
  gap: 12px;
}
.pi-logo-frame {
  display: grid;
  width: 42px;
  height: 42px;
  flex: 0 0 auto;
  place-items: center;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: #fff;
  box-shadow: 0 8px 18px rgba(7, 71, 97, 0.08);
}
.pi-logo,
.pi-logo-fallback {
  width: 28px;
  height: 28px;
  flex: 0 0 auto;
  object-fit: contain;
}
.pi-logo-fallback {
  display: grid;
  place-items: center;
  border-radius: 8px;
  background: var(--pi-primary);
  color: #fff;
  font-weight: 800;
}
.pi-heading {
  min-width: 0;
}
.pi-eyebrow,
.pi-label {
  display: block;
  margin: 0 0 3px;
  color: var(--pi-muted);
  font-size: 11px;
  font-weight: 800;
  letter-spacing: 0.04em;
  line-height: 1.2;
  text-transform: uppercase;
}
.pi-title {
  margin: 0;
  overflow-wrap: anywhere;
  color: var(--pi-text);
  font-size: 16px;
  font-weight: 800;
  line-height: 1.25;
}
.pi-subtitle {
  margin: 4px 0 0;
  color: var(--pi-muted);
  font-size: 13px;
  line-height: 1.35;
}
.pi-close {
  display: grid;
  width: 38px;
  height: 38px;
  flex: 0 0 auto;
  place-items: center;
  border: 1px solid transparent;
  border-radius: 999px;
  background: transparent;
  color: var(--pi-muted);
  cursor: pointer;
  font-size: 24px;
  line-height: 1;
}
.pi-close:hover,
.pi-close:focus-visible {
  border-color: var(--pi-border);
  background: #fff;
  color: var(--pi-text);
}
.pi-body {
  max-height: calc(100vh - 178px);
  overflow-y: auto;
  background: var(--pi-surface);
}
.pi-inline .pi-body {
  max-height: none;
}
.pi-stack {
  display: grid;
  gap: 16px;
}
.pi-summary {
  display: grid;
  overflow: hidden;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: var(--pi-raised);
}
.pi-summary-hero {
  display: flex;
  align-items: flex-end;
  justify-content: space-between;
  gap: 16px;
  padding: 14px;
  border-bottom: 1px solid var(--pi-border);
  background: linear-gradient(135deg, var(--pi-primary-soft), #ffffff 70%);
}
.pi-summary-hero strong {
  display: block;
  color: var(--pi-primary-strong);
  font-size: 26px;
  font-weight: 850;
  line-height: 1.05;
}
.pi-summary-hero p {
  max-width: 42%;
  margin: 0;
  color: var(--pi-muted);
  font-size: 13px;
  font-weight: 700;
  line-height: 1.35;
  text-align: right;
}
.pi-summary-grid {
  display: grid;
  gap: 0;
  padding: 4px 14px;
}
.pi-row {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 16px;
  min-height: 34px;
  border-bottom: 1px solid rgba(216, 225, 232, 0.72);
  color: var(--pi-muted);
  font-size: 14px;
}
.pi-row:last-child {
  border-bottom: 0;
}
.pi-row strong {
  min-width: 0;
  color: var(--pi-text);
  font-weight: 750;
  overflow-wrap: anywhere;
  text-align: right;
}
.pi-reference strong {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 700;
}
.pi-tabs {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(124px, 1fr));
  gap: 6px;
  padding: 5px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: var(--pi-soft);
}
.pi-tab {
  min-height: 40px;
  border: 1px solid transparent;
  border-radius: 9px;
  background: transparent;
  color: var(--pi-muted);
  cursor: pointer;
  font-size: 13px;
  font-weight: 800;
}
.pi-tab:hover:not([disabled]) {
  color: var(--pi-text);
  background: #fff;
}
.pi-tab[aria-selected="true"] {
  border-color: rgba(7, 71, 97, 0.18);
  background: var(--pi-primary);
  color: #fff;
  box-shadow: 0 8px 18px rgba(7, 71, 97, 0.18);
}
.pi-tab[disabled] {
  cursor: not-allowed;
  opacity: 0.48;
}
.pi-panel {
  display: grid;
  gap: 12px;
}
.pi-panel-title,
.pi-state-title {
  margin: 0;
  color: var(--pi-text);
  font-size: 16px;
  font-weight: 850;
  line-height: 1.25;
}
.pi-bank-box {
  display: grid;
  gap: 10px;
  padding: 14px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: #fff;
  box-shadow: 0 10px 24px rgba(8, 18, 28, 0.05);
}
.pi-bank-intro {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
.pi-bank-intro strong {
  color: var(--pi-primary-strong);
  font-size: 14px;
  font-weight: 850;
  text-align: right;
}
.pi-account {
  display: grid;
  gap: 8px;
  padding: 12px;
  border: 1px solid rgba(7, 71, 97, 0.16);
  border-radius: 12px;
  background: linear-gradient(180deg, var(--pi-primary-soft), #fff);
}
.pi-account > span {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 800;
  letter-spacing: 0.04em;
  text-transform: uppercase;
}
.pi-account-number {
  color: var(--pi-primary-strong);
  font-size: 28px;
  font-weight: 900;
  letter-spacing: 0;
  line-height: 1.1;
  overflow-wrap: anywhere;
}
.pi-bank-grid {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 10px;
}
.pi-detail {
  display: grid;
  gap: 5px;
  min-width: 0;
  padding: 12px;
  border: 1px solid var(--pi-border);
  border-radius: 10px;
  background: var(--pi-soft);
}
.pi-detail span {
  color: var(--pi-muted);
  font-size: 12px;
  font-weight: 750;
}
.pi-detail strong {
  min-width: 0;
  color: var(--pi-text);
  font-size: 14px;
  font-weight: 800;
  overflow-wrap: anywhere;
}
.pi-copy,
.pi-primary,
.pi-secondary {
  min-height: 42px;
  border-radius: 10px;
  cursor: pointer;
  font-weight: 800;
}
.pi-copy {
  justify-self: start;
  border: 1px solid var(--pi-primary);
  background: #fff;
  color: var(--pi-primary);
  padding: 0 14px;
}
.pi-copy:hover,
.pi-secondary:hover {
  border-color: var(--pi-border-strong);
  background: var(--pi-soft);
}
.pi-primary {
  width: 100%;
  border: 0;
  background: var(--pi-primary);
  color: #fff;
  box-shadow: 0 10px 18px rgba(7, 71, 97, 0.18);
}
.pi-primary:hover,
.pi-primary:focus-visible {
  background: var(--pi-primary-strong);
}
.pi-primary:disabled,
.pi-secondary:disabled {
  cursor: not-allowed;
  opacity: 0.62;
  box-shadow: none;
}
.pi-secondary {
  border: 1px solid var(--pi-border);
  background: #fff;
  color: var(--pi-text);
  padding: 0 14px;
}
.pi-secondary:disabled {
  cursor: wait;
}
.pi-status {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
  padding: 11px 12px;
  border: 1px solid rgba(154, 103, 0, 0.2);
  border-radius: 10px;
  background: var(--pi-warning-soft);
  color: #6f4a00;
  font-size: 13px;
}
.pi-status strong {
  color: #5f3f00;
  font-weight: 850;
  text-align: right;
  text-transform: capitalize;
}
.pi-bank-actions {
  display: grid;
}
.pi-refresh {
  width: 100%;
}
.pi-redirect-card {
  display: grid;
  gap: 14px;
  padding: 16px;
  border: 1px solid var(--pi-border);
  border-radius: 12px;
  background: linear-gradient(180deg, #ffffff, var(--pi-soft));
}
.pi-redirect-icon {
  display: grid;
  width: 42px;
  height: 42px;
  place-items: center;
  border-radius: 999px;
  background: var(--pi-primary-soft);
  color: var(--pi-primary);
  font-size: 20px;
  font-weight: 900;
}
.pi-state {
  display: grid;
  justify-items: center;
  gap: 10px;
  padding: 34px 14px;
  text-align: center;
}
.pi-state-logo {
  width: 36px;
  height: 36px;
  object-fit: contain;
}
.pi-spinner {
  width: 34px;
  height: 34px;
  border: 3px solid var(--pi-border);
  border-top-color: var(--pi-primary);
  border-radius: 50%;
  animation: pi-spin 0.8s linear infinite;
}
.pi-badge {
  display: inline-grid;
  place-items: center;
  min-width: 40px;
  height: 40px;
  border-radius: 999px;
  font-weight: 900;
}
.pi-badge-success {
  background: var(--pi-success-soft);
  color: var(--pi-success);
}
.pi-badge-error {
  background: var(--pi-danger-soft);
  color: var(--pi-danger);
}
.pi-badge-warning {
  background: var(--pi-warning-soft);
  color: var(--pi-warning);
}
.pi-message {
  margin: 0;
  color: var(--pi-muted);
  font-size: 14px;
  line-height: 1.5;
}
.pi-error {
  padding: 12px;
  border: 1px solid #fecdca;
  border-radius: 10px;
  background: #fffbfa;
  color: var(--pi-danger);
  font-size: 14px;
}
.pi-unavailable {
  padding: 14px;
  border: 1px dashed var(--pi-border-strong);
  border-radius: 10px;
  background: var(--pi-soft);
  color: var(--pi-muted);
  font-size: 14px;
}
.pi-footer {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
  border-top: 1px solid var(--pi-border);
  background: var(--pi-raised);
}
.pi-secured {
  display: inline-flex;
  align-items: center;
  min-width: 0;
  gap: 7px;
  padding: 7px 10px;
  border-radius: 999px;
  background: var(--pi-primary-strong);
  color: rgba(255, 255, 255, 0.78);
  font-size: 12px;
  line-height: 1;
}
.pi-secured strong {
  color: #fff;
  font-weight: 850;
}
.pi-secured-logo {
  width: 15px;
  height: 20px;
  object-fit: contain;
}
.pi-sr-only {
  position: absolute;
  width: 1px;
  height: 1px;
  padding: 0;
  overflow: hidden;
  clip: rect(0, 0, 0, 0);
  white-space: nowrap;
  border: 0;
}
@keyframes pi-spin {
  to {
    transform: rotate(360deg);
  }
}
@media (max-width: 520px) {
  .pi-overlay {
    align-items: end;
    padding: 10px;
  }
  .pi-modal {
    width: 100%;
    max-height: calc(100vh - 20px);
    border-radius: 14px 14px 10px 10px;
  }
  .pi-header,
  .pi-body,
  .pi-footer {
    padding: 15px;
  }
  .pi-header {
    gap: 10px;
  }
  .pi-logo-frame {
    width: 38px;
    height: 38px;
    border-radius: 10px;
  }
  .pi-title {
    font-size: 15px;
  }
  .pi-summary-hero {
    align-items: flex-start;
    flex-direction: column;
    gap: 8px;
  }
  .pi-summary-hero p {
    max-width: none;
    text-align: left;
  }
  .pi-summary-hero strong {
    font-size: 25px;
  }
  .pi-bank-intro,
  .pi-status {
    align-items: flex-start;
    flex-direction: column;
    gap: 6px;
  }
  .pi-bank-intro strong,
  .pi-status strong {
    text-align: left;
  }
  .pi-bank-grid {
    grid-template-columns: 1fr;
  }
  .pi-account-number {
    font-size: 26px;
  }
  .pi-tabs {
    grid-template-columns: 1fr;
  }
  .pi-footer {
    align-items: stretch;
    flex-direction: column;
  }
  .pi-secured {
    justify-content: center;
  }
}
`;var X=["a[href]","button:not([disabled])","input:not([disabled])","select:not([disabled])","textarea:not([disabled])",'[tabindex]:not([tabindex="-1"])'].join(","),x=class{constructor(e){this.inline=!1;this.refreshingStatus=!1;this.options=e,this.host=document.createElement("div"),this.host.setAttribute("data-payisland-checkout",""),this.root=this.host.attachShadow({mode:"open"})}mount(){this.previousActiveElement=document.activeElement;let e=this.resolveContainer();this.inline=!!e,(e??document.body).appendChild(this.host),this.renderShell(),this.focusFirst()}destroy(){this.stopCountdown(),this.host.remove(),this.previousActiveElement instanceof HTMLElement&&this.previousActiveElement.focus({preventScroll:!0})}renderLoading(){this.setBody(`
      <div class="pi-state pi-state-loading" role="status" aria-live="polite">
        <img class="pi-state-logo" src="${f(S)}" alt="" />
        <div class="pi-spinner" aria-hidden="true"></div>
        <h3 class="pi-state-title">Preparing checkout</h3>
        <p class="pi-message">Preparing secure checkout...</p>
      </div>
    `)}getSelectedChannel(){return this.selectedChannel}renderError(e,r=!0){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        <div class="pi-badge pi-badge-error" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Checkout unavailable</h3>
        <p class="pi-message">${a(e.message)}</p>
        ${r?'<button class="pi-secondary" type="button" data-action="retry">Retry</button>':""}
      </div>
    `)}renderCheckout(e,r,i){let n=this.filterChannels(B(e),r,e);return n.length===0?(this.selectedChannel=void 0,this.renderError({code:"no_available_channels",message:"No available payment channels for this checkout."},!1),!1):(this.selectedChannel=this.selectedChannel&&n.includes(this.selectedChannel)?this.selectedChannel:n.find(o=>this.isSupported(o,e))??n[0],this.setBody(`
      <div class="pi-stack">
        ${this.renderSummary(e)}
        ${this.renderTabs(n,e)}
        <div class="pi-panel">${this.renderChannel(e,this.selectedChannel,i)}</div>
      </div>
    `),this.startCountdown(e),!0)}renderPending(e){this.setBody(`
      <div class="pi-state pi-state-pending" role="status" aria-live="polite">
        <div class="pi-spinner" aria-hidden="true"></div>
        <h3 class="pi-state-title">Confirming payment</h3>
        <p class="pi-message">Payment is pending. We are checking for confirmation.</p>
        ${e?.status?`<p class="pi-subtitle">Status: ${a(e.status)}</p>`:""}
      </div>
    `)}renderSuccess(){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-success" role="status" aria-live="polite">
        <div class="pi-badge pi-badge-success" aria-hidden="true">\u2713</div>
        <h3 class="pi-state-title">Payment successful</h3>
        <p class="pi-message">Payment successful.</p>
      </div>
    `)}renderFailure(e){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        <div class="pi-badge pi-badge-error" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Payment failed</h3>
        <p class="pi-message">${a(e)}</p>
      </div>
    `)}renderExpired(){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-expired" role="alert">
        <div class="pi-badge pi-badge-warning" aria-hidden="true">!</div>
        <h3 class="pi-state-title">Checkout expired</h3>
        <p class="pi-message">This checkout has expired. Please start a new payment.</p>
      </div>
    `)}setStatusRefreshing(e){this.refreshingStatus=e;let r=this.root.querySelector('[data-action="refresh-status"]');r&&(r.disabled=e,r.textContent=e?"Checking...":"Refresh status")}renderShell(){let e=this.options.theme.merchantName??"PayIsland Checkout",r=this.options.theme.logoUrl??S,i=this.inline?"pi-inline":"pi-overlay";this.root.innerHTML=`
      <style>${q}</style>
      <div class="${i}" data-shell>
        <section class="pi-modal" role="dialog" aria-modal="${this.inline?"false":"true"}" aria-labelledby="pi-title">
          <header class="pi-header">
            <div class="pi-header-main">
              <div class="pi-logo-frame">
                <img class="pi-logo" src="${f(r)}" alt="" />
              </div>
              <div class="pi-heading">
                <p class="pi-eyebrow">Secure checkout</p>
                <h2 class="pi-title" id="pi-title">${a(e)}</h2>
                <p class="pi-subtitle">Protected by PayIsland</p>
              </div>
            </div>
            <button class="pi-close" type="button" aria-label="Close checkout" data-action="close">
              <span aria-hidden="true">\xD7</span>
            </button>
          </header>
          <main class="pi-body" data-body></main>
          <footer class="pi-footer">
            <div class="pi-secured">
              <span>Secured by</span>
              <img class="pi-secured-logo" src="${f(H)}" alt="PayIsland" />
              <strong>PayIsland</strong>
            </div>
            <button class="pi-secondary" type="button" data-action="close">Cancel</button>
          </footer>
        </section>
      </div>
    `,this.root.querySelector("[data-shell]")?.addEventListener("click",n=>{!this.inline&&n.target===n.currentTarget&&this.options.onClose("user")}),this.root.addEventListener("click",n=>this.handleClick(n)),this.root.addEventListener("keydown",n=>this.handleKeydown(n)),this.options.theme.primaryColor&&this.host.style.setProperty("--pi-primary",this.options.theme.primaryColor)}setBody(e){let r=this.root.querySelector("[data-body]");r&&(r.innerHTML=e)}renderSummary(e){let r=l(e),i=K(e),n=A(e),o=r.currency??e.currency??"NGN",c=r.total_amount??r.totalAmount??e.total_amount??e.totalAmount,p=r.amount??e.amount,h=r.fee??e.fee,L=F(n),O=U(n.email),ne=J(i,this.options.theme.merchantName),z=r.reference??e.reference??"",ie=b(c??p,o);return`
      <section class="pi-summary" aria-label="Transaction summary">
        <div class="pi-summary-hero">
          <div>
            <span class="pi-label">Total due</span>
            <strong>${a(ie)}</strong>
          </div>
          <p>${a(ne)}</p>
        </div>
        <div class="pi-summary-grid">
          <div class="pi-row"><span>Amount</span><strong>${a(b(p,o))}</strong></div>
          <div class="pi-row"><span>Fee</span><strong>${a(b(h,o))}</strong></div>
          ${L?`<div class="pi-row"><span>Customer</span><strong>${a(L)}</strong></div>`:""}
          ${O?`<div class="pi-row"><span>Email</span><strong>${a(O)}</strong></div>`:""}
          ${z?`<div class="pi-row pi-reference"><span>Reference</span><strong>${a(z)}</strong></div>`:""}
        </div>
      </section>
    `}renderTabs(e,r){return`
      <div class="pi-tabs" role="tablist" aria-label="Payment channels">
        ${e.map(i=>{let n=this.isSupported(i,r);return`
              <button
                class="pi-tab"
                type="button"
                role="tab"
                aria-selected="${i===this.selectedChannel?"true":"false"}"
                data-channel="${f(i)}"
                ${n?"":"disabled"}
              >
                ${a(Y(i))}
              </button>
            `}).join("")}
      </div>
    `}renderChannel(e,r,i){if(r==="bank-transfer"){let n=E(u(e)),o=l(e),c=o.currency??e.currency??"NGN",p=o.total_amount??o.totalAmount??e.total_amount??e.totalAmount??o.amount??e.amount;return n?`
        <section class="pi-bank-box" aria-label="Bank transfer details">
          <div class="pi-bank-intro">
            <span class="pi-label">Bank transfer</span>
            <strong>Transfer exactly ${a(b(p,c))}</strong>
          </div>
          <div class="pi-account">
            <span>Account number</span>
            <strong class="pi-account-number">${a(n.accountNumber)}</strong>
            <button class="pi-copy" type="button" data-copy="${f(n.accountNumber)}">Copy number</button>
          </div>
          <div class="pi-bank-grid">
            <div class="pi-detail"><span>Bank</span><strong>${a(n.bankName)}</strong></div>
            <div class="pi-detail"><span>Account name</span><strong>${a(n.accountName)}</strong></div>
          </div>
          ${n.expiresAt?`<p class="pi-subtitle" data-countdown="${f(n.expiresAt)}"></p>`:""}
          <div class="pi-status" role="status" aria-live="polite">
            <span>Current status</span>
            <strong>${a(ve(i))}</strong>
          </div>
          <div class="pi-bank-actions">
            <button class="pi-secondary pi-refresh" type="button" data-action="refresh-status" ${this.refreshingStatus?"disabled":""}>
              ${this.refreshingStatus?"Checking...":"Refresh status"}
            </button>
          </div>
          <p class="pi-message">Keep this checkout open while we confirm your payment.</p>
        </section>
      `:'<div class="pi-unavailable">Bank transfer details are not available yet.</div>'}if(r==="redirect"||m(g(e))){let n=m(g(e));return`
        <section class="pi-redirect-card">
          <div class="pi-redirect-icon" aria-hidden="true">\u2197</div>
          <div>
            <h3 class="pi-panel-title">Continue securely</h3>
            <p class="pi-message">You will continue to a secure PayIsland payment page. Keep this checkout open while we confirm the payment.</p>
          </div>
          <button class="pi-primary" type="button" data-redirect="${f(n??"")}" ${n?"":"disabled"}>
            Continue to payment
          </button>
        </section>
      `}return r==="card"?`<div class="pi-unavailable">${a(W())}</div>`:`<div class="pi-unavailable">${a(Y(r??"This channel"))} is not available in this checkout yet.</div>`}startCountdown(e){this.stopCountdown();let r=E(u(e));if(!r?.expiresAt)return;let i=new Date(r.expiresAt).getTime(),n=this.root.querySelector("[data-countdown]");if(!Number.isFinite(i)||!n)return;let o=()=>{let c=Math.max(0,i-Date.now()),p=Math.floor(c/6e4),h=Math.floor(c%6e4/1e3);n.textContent=c>0?`Transfer account expires in ${p}:${h.toString().padStart(2,"0")}`:"Transfer account has expired."};o(),this.countdownTimer=window.setInterval(o,1e3)}stopCountdown(){this.countdownTimer&&(window.clearInterval(this.countdownTimer),this.countdownTimer=void 0)}filterChannels(e,r,i){let n=[...new Set(e)];return r?.length?n.filter(o=>r.includes(o)&&(i?this.isSupported(o,i):!0)):n.length>0?n:["bank-transfer","redirect","card"]}isSupported(e,r){return e==="bank-transfer"?!!u(r):e==="redirect"?!!m(g(r)):!1}handleClick(e){let r=e.target;if(!(r instanceof HTMLElement))return;if(r.closest('[data-action="close"]')){this.options.onClose("user");return}if(r.closest('[data-action="retry"]')){this.options.onRetry();return}if(r.closest('[data-action="refresh-status"]')){this.options.onRefreshStatus();return}let c=r.closest("[data-channel]")?.dataset.channel;if(c){this.selectedChannel=c,this.options.onChannelSelected(c);return}let p=r.closest("[data-copy]")?.dataset.copy;if(p){navigator.clipboard?.writeText(p),r.textContent="Copied";return}let h=r.closest("[data-redirect]")?.dataset.redirect;h&&V(h)}handleKeydown(e){if(e.key==="Escape"){this.options.onClose("user");return}if(e.key!=="Tab"||this.inline)return;let r=Array.from(this.root.querySelectorAll(X));if(r.length===0)return;let i=r[0],n=r[r.length-1],o=this.root.activeElement;e.shiftKey&&o===i?(e.preventDefault(),n.focus()):!e.shiftKey&&o===n&&(e.preventDefault(),i.focus())}focusFirst(){window.setTimeout(()=>{this.root.querySelector(X)?.focus()},0)}resolveContainer(){let e=this.options.container;if(e)return e instanceof HTMLElement?e:document.querySelector(e)??void 0}};function Y(t){return{"bank-transfer":"Bank transfer",redirect:"Redirect",card:"Card",mono:"Mono"}[t]??String(t)}function ve(t){if(!t)return"Pending confirmation";let e=String(t).trim();return e?e.replace(/[-_]+/g," "):"Pending confirmation"}function a(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function f(t){return a(t)}var v=class{constructor(){this.stopped=!0}start(e,r,i=0){this.stop(),this.stopped=!1;let n=async()=>{if(!this.stopped)try{let o=await e();if(this.stopped||(r(o),this.stopped))return;this.timer=window.setTimeout(n,y(o))}catch{this.stopped||(this.timer=window.setTimeout(n,5e3))}};this.timer=window.setTimeout(n,i)}stop(){this.stopped=!0,this.timer&&(window.clearTimeout(this.timer),this.timer=void 0)}};var we={idle:{LOAD:"loading",CLOSE:"closed"},loading:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},ready:{PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},pending:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},success:{CLOSE:"closed"},failed:{CLOSE:"closed"},expired:{CLOSE:"closed"},closed:{}},w=class{constructor(e){this.context=e}send(e){let r=we[this.context.status][e.type];return r&&(this.context.status=r),this.context.status}isTerminal(){return this.context.status==="success"||this.context.status==="failed"||this.context.status==="expired"||this.context.status==="closed"}};var s;function ee(t){k(!1);let e;try{e=M(t?.reference)}catch(c){let p=I(c);t?.onError?.(p),Q(t,p);return}let r;try{r=new C(N(t.__apiBaseUrl))}catch(c){let p=I(c);t.onError?.(p),Q(t,p);return}let i=new w({reference:e,theme:t.theme??{},status:"idle"}),n=new v,o=new x({container:t.container,theme:t.theme??{},onClose:c=>k(c==="user"),onRetry:()=>{Z()},onChannelSelected:()=>{s?.bootstrap&&s.modal.renderCheckout(s.bootstrap,s.options.channels,s.machine.context.status)},onRefreshStatus:()=>{ke()}});s={options:t,reference:e,api:r,modal:o,poller:n,machine:i,successCalled:!1,errorCalled:!1,closeCalled:!1},o.mount(),Z()}function k(t=!0){if(!s)return;let e=s;s=void 0,e.poller.stop(),e.machine.send({type:"CLOSE"}),e.modal.destroy(),e.api?.setCheckoutToken(void 0),e.bootstrap=void 0,t&&!e.closeCalled&&(e.closeCalled=!0,e.options.onClose?.())}async function Z(){let t=s;if(!(!t||!t.api)){t.poller.stop(),t.machine.send({type:"LOAD"}),t.modal.renderLoading();try{let e=await t.api.bootstrap(t.reference);if(s!==t)return;if(t.bootstrap=e.data,t.machine.context.bootstrap=e.data,t.machine.context.checkoutToken=e.checkoutToken,t.api.setCheckoutToken(e.checkoutToken),!t.modal.renderCheckout(e.data,t.options.channels)){t.machine.send({type:"FAIL"}),T({code:"no_available_channels",message:"No available payment channels for this checkout."});return}D(e.data,!1);let i=R(P(e.data));(i==="unknown"||i==="pending")&&_(y(e.data))}catch(e){if(s!==t)return;let r=I(e);t.machine.context.error=r,t.machine.send({type:"FAIL"}),t.modal.renderError(r),T(r)}}}function _(t){let e=s;!e||!e.api||e.poller.start(()=>te(e),r=>D(r,!0),t)}async function ke(){let t=s;if(!(!t||!t.api)){t.poller.stop(),t.modal.setStatusRefreshing(!0);try{let e=await te(t);if(s!==t)return;D(e,!0),t.machine.isTerminal()||_(y(e))}catch{s===t&&!t.machine.isTerminal()&&_(5e3)}finally{s===t&&t.modal.setStatusRefreshing(!1)}}}async function te(t){if(!t.api)return{};if(t.modal.getSelectedChannel()==="bank-transfer"&&u(t.bootstrap))try{return(await t.api.verifyBankTransfer(t.reference)).data}catch{return(await t.api.verify(t.reference)).data}return(await t.api.verify(t.reference)).data}function D(t,e){let r=s;if(!r)return;let i=R(P(t)),n=l(t);if(i==="success"){r.poller.stop(),r.machine.send({type:"SUCCESS"}),r.modal.renderSuccess(),r.successCalled||(r.successCalled=!0,r.options.onSuccess?.(n));return}if(i==="expired"){r.poller.stop(),r.machine.send({type:"EXPIRE"}),r.modal.renderExpired(),T({code:"payment_expired",message:"This checkout has expired. Please start a new payment."});return}if(i==="failed"){r.poller.stop(),r.machine.send({type:"FAIL"}),r.modal.renderFailure("This payment could not be completed."),T({code:"payment_failed",message:"This payment could not be completed."});return}e||i==="pending"?(r.machine.send({type:"PENDING"}),r.modal.getSelectedChannel()==="bank-transfer"&&r.bootstrap&&u(r.bootstrap)?r.modal.renderCheckout(r.bootstrap,r.options.channels,P(t)??"pending"):r.modal.renderPending(t),r.options.onPending?.(t)):r.machine.send({type:"READY"})}function T(t){let e=s;!e||e.errorCalled||(e.errorCalled=!0,e.options.onError?.(t))}function Q(t,e){let r=new x({container:t?.container,theme:t?.theme??{},onClose:n=>k(n==="user"),onRetry:()=>{},onChannelSelected:()=>{},onRefreshStatus:()=>{}}),i=new w({reference:"",theme:t?.theme??{},status:"failed"});s={options:t??{reference:""},reference:"",modal:r,poller:new v,machine:i,successCalled:!1,errorCalled:!0,closeCalled:!1},r.mount(),r.renderError(e,!1)}function I(t){return t instanceof d?t.toPayload():{code:"checkout_error",message:"We could not continue with this checkout."}}var re={open:ee,close:k};typeof window<"u"&&(window.PayIslandCheckout=re);var Pe=re;return de(Ce);})();
