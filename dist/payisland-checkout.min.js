...
`;var Q=["a[href]","button:not([disabled])","input:not([disabled])","select:not([disabled])","textarea:not([disabled])",'[tabindex]:not([tabindex="-1"])'].join(","),v=class{constructor(e){this.inline=!1;this.refreshingStatus=!1;this.options=e,this.host=document.createElement("div"),this.host.setAttribute("data-payisland-checkout",""),this.root=this.host.attachShadow({mode:"open"})}mount(){this.previousActiveElement=document.activeElement;let e=this.resolveContainer();this.inline=!!e,(e??document.body).appendChild(this.host),this.renderShell(),this.focusFirst()}destroy(){this.stopCountdown(),this.host.remove(),this.previousActiveElement instanceof HTMLElement&&this.previousActiveElement.focus({preventScroll:!0})}renderLoading(){this.setBody(`
    ...
    `)}getSelectedChannel(){return this.selectedChannel}renderError(e,n=!0){this.stopCountdown(),this.setBody(`
      <div class="pi-state pi-state-error" role="alert">
        ...
        <p class="pi-message">${s(e.message)}</p>
        ${n?'<button class="pi-secondary" type="button" data-action="retry">Retry</button>':""}
      </div>
    `)}renderCheckout(e,n,i){let r=this.filterChannels(U(e),n,e);return r.length===0?(this.selectedChannel=void 0,this.renderError({code:"no_available_channels",message:"No available payment channels for this checkout."},!1),!1):(this.selectedChannel=this.selectedChannel&&r.includes(this.selectedChannel)?this.selectedChannel:r.find(o=>this.isSupported(o,e))??r[0],this.setBody(`
      <div class="pi-stack">
        ${this.renderSummary(e)}
        ${this.renderTabs(r,e)}
        <div class="pi-panel">${this.renderChannel(e,this.selectedChannel,i)}</div>
      </div>
    `),this.startCountdown(e),!0)}renderPending(e){this.setBody(`
      <div class="pi-state pi-state-pending" role="status" aria-live="polite">
        ...
        ${e?.status?`<p class="pi-subtitle">Status: ${s(e.status)}</p>`:""}
      </div>
        ...
        <section class="pi-modal" role="dialog" aria-modal="${this.inline?"false":"true"}" aria-labelledby="pi-title">
          <header class="pi-header">
            <div class="pi-header-main">
              ...
              <div class="pi-heading">
                ...
                <h2 class="pi-title" id="pi-title">${s(e)}</h2>
          ...
          <footer class="pi-footer">
            <div class="pi-secured">
              ...
              <img class="pi-secured-logo" src="${f(_)}" alt="PayIsland" />
    ...
    `,this.root.querySelector("[data-shell]")?.addEventListener("click",r=>{!this.inline&&r.target===r.currentTarget&&this.options.onClose("user")}),this.root.addEventListener("click",r=>this.handleClick(r)),this.root.addEventListener("keydown",r=>this.handleKeydown(r)),this.options.theme.primaryColor&&this.host.style.setProperty("--pi-primary",this.options.theme.primaryColor)}setBody(e){let n=this.root.querySelector("[data-body]");n&&(n.innerHTML=e)}renderSummary(e){let n=l(e),i=F(e),r=J(e),o=n.currency??e.currency??"NGN",c=n.total_amount??n.totalAmount??e.total_amount??e.totalAmount,d=n.amount??e.amount,h=n.fee??e.fee,L=W(r),N=G(r.email),ae=V(i,this.options.theme.merchantName),z=n.reference??e.reference??"",se=x(c??d,o);return`
      <section class="pi-summary" aria-label="Transaction summary">
        ...
        <div class="pi-summary-grid">
          <div class="pi-row"><span>Amount</span><strong>${s(x(d,o))}</strong></div>
          <div class="pi-row"><span>Fee</span><strong>${s(x(h,o))}</strong></div>
          ${L?`<div class="pi-row"><span>Customer</span><strong>${s(L)}</strong></div>`:""}
          ${N?`<div class="pi-row"><span>Email</span><strong>${s(N)}</strong></div>`:""}
          ${z?`<div class="pi-row pi-reference"><span>Reference</span><strong>${s(z)}</strong></div>`:""}
        </div>
      </section>
    `}renderTabs(e,n){return`
      <div class="pi-tabs" role="tablist" aria-label="Payment channels">
        ${e.map(i=>{let r=this.isSupported(i,n);return`
              <button
                class="pi-tab"
                type="button"
                role="tab"
                aria-selected="${i===this.selectedChannel?"true":"false"}"
                data-channel="${f(i)}"
                ${r?"":"disabled"}
              >
                ${s(ee(i))}
    ...
    `}renderChannel(e,n,i){if(n==="bank-transfer"){let r=E(u(e)),o=l(e),c=o.currency??e.currency??"NGN",d=o.total_amount??o.totalAmount??e.total_amount??e.totalAmount??o.amount??e.amount;return r?`
        <section class="pi-bank-box" aria-label="Bank transfer details">
          <div class="pi-bank-intro">
            <span class="pi-label">Bank transfer</span>
            <strong>Transfer exactly ${s(x(d,c))}</strong>
          </div>
          <div class="pi-account">
            <span>Account number</span>
            <strong class="pi-account-number">${s(r.accountNumber)}</strong>
            <button class="pi-copy" type="button" data-copy="${f(r.accountNumber)}">Copy number</button>
          </div>
          <div class="pi-bank-grid">
            <div class="pi-detail"><span>Bank</span><strong>${s(r.bankName)}</strong></div>
            <div class="pi-detail"><span>Account name</span><strong>${s(r.accountName)}</strong></div>
          </div>
          ${r.expiresAt?`<p class="pi-subtitle" data-countdown="${f(r.expiresAt)}"></p>`:""}
          <div class="pi-status" role="status" aria-live="polite">
            <span>Current status</span>
            <strong>${s(Pe(i))}</strong>
          </div>
      ...
      `:'<div class="pi-unavailable">Bank transfer details are not available yet.</div>'}if(n==="redirect"||m(y(e))){let r=m(y(e));return`
        <section class="pi-redirect-card">
          ...
          <button class="pi-primary" type="button" data-redirect="${f(r??"")}" ${r?"":"disabled"}>
            Continue to payment
          </button>
        </section>
      `}return n==="card"?`<div class="pi-unavailable">${s(X())}</div>`:`<div class="pi-unavailable">${s(ee(n??"This channel"))} is not available in this checkout yet.</div>`}startCountdown(e){this.stopCountdown();let n=E(u(e));if(!n?.expiresAt)return;let i=new Date(n.expiresAt).getTime(),r=this.root.querySelector("[data-countdown]");if(!Number.isFinite(i)||!r)return;let o=()=>{let c=Math.max(0,i-Date.now()),d=Math.floor(c/6e4),h=Math.floor(c%6e4/1e3);r.textContent=c>0?`Transfer account expires in ${d}:${h.toString().padStart(2,"0")}`:"Transfer account has expired."};o(),this.countdownTimer=window.setInterval(o,1e3)}stopCountdown(){this.countdownTimer&&(window.clearInterval(this.countdownTimer),this.countdownTimer=void 0)}filterChannels(e,n,i){let r=[...new Set(e)];return n?.length?r.filter(o=>n.includes(o)&&(i?this.isSupported(o,i):!0)):r.length>0?r:["bank-transfer","redirect","card"]}isSupported(e,n){return e==="bank-transfer"?!!u(n):e==="redirect"?!!m(y(n)):!1}handleClick(e){let n=e.target;if(!(n instanceof HTMLElement))return;if(n.closest('[data-action="close"]')){this.options.onClose("user");return}if(n.closest('[data-action="retry"]')){this.options.onRetry();return}if(n.closest('[data-action="refresh-status"]')){this.selectedChannel&&this.options.onPaymentStarted(this.selectedChannel),this.options.onRefreshStatus();return}let c=n.closest("[data-channel]")?.dataset.channel;if(c){this.selectedChannel=c,this.options.onChannelSelected(c);return}let d=n.closest("[data-copy]")?.dataset.copy;if(d){navigator.clipboard?.writeText(d),n.textContent="Copied";return}let h=n.closest("[data-redirect]")?.dataset.redirect;h&&(Y(h),this.options.onPaymentStarted(this.selectedChannel??"redirect"))}handleKeydown(e){if(e.key==="Escape"){this.options.onClose("user");return}if(e.key!=="Tab"||this.inline)return;let n=Array.from(this.root.querySelectorAll(Q));if(n.length===0)return;let i=n[0],r=n[n.length-1],o=this.root.activeElement;e.shiftKey&&o===i?(e.preventDefault(),r.focus()):!e.shiftKey&&o===r&&(e.preventDefault(),i.focus())}focusFirst(){window.setTimeout(()=>{this.root.querySelector(Q)?.focus()},0)}resolveContainer(){let e=this.options.container;if(e)return e instanceof HTMLElement?e:document.querySelector(e)??void 0}};function ee(t){return{"bank-transfer":"Bank transfer",redirect:"Redirect",card:"Card",mono:"Mono"}[t]??String(t)}function Pe(t){if(!t)return"Pending confirmation";let e=String(t).trim();return e?e.replace(/[-_]+/g," "):"Pending confirmation"}function s(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function f(t){return s(t)}var w=class{constructor(){this.stopped=!0}start(e,n,i=0){this.stop(),this.stopped=!1;let r=async()=>{if(!this.stopped)try{let o=await e();if(this.stopped||(n(o),this.stopped))return;this.timer=window.setTimeout(r,b(o))}catch{this.stopped||(this.timer=window.setTimeout(r,5e3))}};this.timer=window.setTimeout(r,i)}stop(){this.stopped=!0,this.timer&&(window.clearTimeout(this.timer),this.timer=void 0)}};var Ce={idle:{LOAD:"loading",CLOSE:"closed"},loading:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},ready:{PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},pending:{READY:"ready",PENDING:"pending",SUCCESS:"success",FAIL:"failed",EXPIRE:"expired",CLOSE:"closed"},success:{CLOSE:"closed"},failed:{CLOSE:"closed"},expired:{CLOSE:"closed"},closed:{}},k=class{constructor(e){this.context=e}send(e){let n=Ce[this.context.status][e.type];return n&&(this.context.status=n),this.context.status}isTerminal(){return this.context.status==="success"||this.context.status==="failed"||this.context.status==="expired"||this.context.status==="closed"}};var a;function re(t){P(!1);let e;try{e=j(t?.reference)}catch(c){let d=I(c);t?.onError?.(d),ne(t,d);return}let n;try{n=new C(B(t.__apiBaseUrl))}catch(c){let d=I(c);t.onError?.(d),ne(t,d);return}let i=new k({reference:e,theme:t.theme??{},status:"idle"}),r=new w,o=new v({container:t.container,theme:t.theme??{},onClose:c=>P(c==="user"),onRetry:()=>{te()},onChannelSelected:()=>{a?.bootstrap&&a.modal.renderCheckout(a.bootstrap,a.options.channels,a.machine.context.status)},onPaymentStarted:()=>Se(),onRefreshStatus:()=>{Te()}});a={options:t,reference:e,api:n,modal:o,poller:r,machine:i,successCalled:!1,errorCalled:!1,closeCalled:!1},o.mount(),te()}function P(t=!0){if(!a)return;let e=a;a=void 0,e.poller.stop(),e.machine.send({type:"CLOSE"}),e.modal.destroy(),e.api?.setCheckoutToken(void 0),e.bootstrap=void 0,t&&!e.closeCalled&&(e.closeCalled=!0,e.options.onClose?.())}async function te(){let t=a;if(!(!t||!t.api)){t.poller.stop(),t.machine.send({type:"LOAD"}),t.modal.renderLoading();try{let e=await t.api.bootstrap(t.reference);if(a!==t)return;if(t.bootstrap=e.data,t.machine.context.bootstrap=e.data,t.machine.context.checkoutToken=e.checkoutToken,t.api.setCheckoutToken(e.checkoutToken),!t.modal.renderCheckout(e.data,t.options.channels)){t.machine.send({type:"FAIL"}),T({code:"no_available_channels",message:"No available payment channels for this checkout."});return}D(e.data,!1)}catch(e){if(a!==t)return;let n=I(e);t.machine.context.error=n,t.machine.send({type:"FAIL"}),t.modal.renderError(n),T(n)}}}function Se(){let t=a;!t||!t.bootstrap||t.machine.isTerminal()||H(b(t.bootstrap))}function H(t){let e=a;!e||!e.api||e.poller.start(()=>ie(e),n=>D(n,!0),t)}async function Te(){let t=a;if(!(!t||!t.api)){t.poller.stop(),t.modal.setStatusRefreshing(!0);try{let e=await ie(t);if(a!==t)return;D(e,!0),t.machine.isTerminal()||H(b(e))}catch{a===t&&!t.machine.isTerminal()&&H(5e3)}finally{a===t&&t.modal.setStatusRefreshing(!1)}}}async function ie(t){if(!t.api)return{};if(t.modal.getSelectedChannel()==="bank-transfer"&&u(t.bootstrap))try{return(await t.api.verifyBankTransfer(t.reference)).data}catch{return(await t.api.verify(t.reference)).data}return(await t.api.verify(t.reference)).data}function D(t,e){let n=a;if(!n)return;let i=K(R(t)),r=l(t);if(i==="success"){n.poller.stop(),n.machine.send({type:"SUCCESS"}),n.modal.renderSuccess(),n.successCalled||(n.successCalled=!0,n.options.onSuccess?.(r));return}if(i==="expired"){n.poller.stop(),n.machine.send({type:"EXPIRE"}),n.modal.renderExpired(),T({code:"payment_expired",message:"This checkout has expired. Please start a new payment."});return}if(i==="failed"){n.poller.stop(),n.machine.send({type:"FAIL"}),n.modal.renderFailure("This payment could not be completed."),T({code:"payment_failed",message:"This payment could not be completed."});return}e?(n.machine.send({type:"PENDING"}),n.bootstrap?n.modal.renderCheckout(n.bootstrap,n.options.channels,R(t)??i):n.modal.renderPending(t),n.options.onPending?.(t)):n.machine.send({type:"READY"})}function T(t){let e=a;!e||e.errorCalled||(e.errorCalled=!0,e.options.onError?.(t))}function ne(t,e){let n=new v({container:t?.container,theme:t?.theme??{},onClose:r=>P(r==="user"),onRetry:()=>{},onChannelSelected:()=>{},onPaymentStarted:()=>{},onRefreshStatus:()=>{}}),i=new k({reference:"",theme:t?.theme??{},status:"failed"});a={options:t??{reference:""},reference:"",modal:n,poller:new w,machine:i,successCalled:!1,errorCalled:!0,closeCalled:!1},n.mount(),n.renderError(e,!1)}function I(t){return t instanceof p?t.toPayload():{code:"checkout_error",message:"We could not continue with this checkout."}}var oe={open:re,close:P};typeof window<"u"&&(window.PayIslandCheckout=oe);var $e=oe;return fe(Re);})();
